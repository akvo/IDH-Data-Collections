...
setwd("/Users/ineslesire/Documents/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/5217 Goshen/Mango")
##  ---- Describe datafiles ----
##DOWNLOAD THESE ALL TOGETHER IN A FOLDER
## Rename your files
pi_filename <- "pi_2022_goshen_mango.xlsx"
data_filename <- "404080053_goshen_mango_rawdata.xlsx"
survey_filename <- "survey_goshen_mango.xlsx"
data_delivery <- "2022_Goshen_anom_mango.xlsx"
...
# Context specific adjustments
  ...
  # More units sold than produced --> delete entries cause they don't make sense
  filter(identifier != "3qf46hxh0xmu") %>%
  filter(identifier != "v55h9qh8fmqg") %>%
  filter(identifier != "d32nbtt99j7w") %>%
  filter(identifier != "q5jc5t91sg24") %>%
  
  # Unrealistic prices --> some farmers indicated total income instead of price per unit
  mutate(f_focus_price = ifelse(f_focus_price == 60000, 3, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 54000, 6.75, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 20000, 10, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 10000, 4, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 9800, 7, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 9100, 6, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 9000, NA, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 1500, NA, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 1200 & identifier == "apx63j38x0d1", NA, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 800 & identifier != "ws0abxeapesh", NA, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 700, NA, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 200, 20, f_focus_price)) %>%
  
  # Strange prices replace by median
  mutate(f_focus_price = ifelse(f_focus_price == 3000, 15, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 1300, 15, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 1200, 15, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 1000, 15, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 800, 15, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(f_focus_price == 300 & identifier == "ws0abxeapesh", 15, f_focus_price)) %>%
  
  mutate(f_focus_measurement_prod = ifelse(is.na(f_focus_measurement_prod), "per piece", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_sold = ifelse(is.na(f_focus_measurement_sold), "per piece", f_focus_measurement_sold))
...
Data <- Data %>%
  ...
  ##Check what the common measurement unit for land is and adjust the following section accordingly. 
...
Data <- Data %>%
  ...
  mutate(cal_focus_measurement_prod = ifelse(f_focus_measurement_prod == "per piece", 1, cal_focus_measurement_prod)) %>%
  mutate(cal_focus_measurement_prod = ifelse(f_focus_measurement_prod %in% c("have not harvested", "did not harvest", "never sold",
                                           "i dont know", "depends with size of banana"), NA, cal_focus_measurement_prod))
  
  ##Measurement unit for sales 

Data <- Data %>%
  # first make a new f_focus_measurement_sold variable that cleans the different "per piece" options
  mutate(f_focus_measurement_sold_2 = ifelse(grepl("piece", f_focus_measurement_sold), "per piece",f_focus_measurement_sold)) %>%
  
  mutate(cal_focus_measurement_sold := ifelse(
    grepl("[[:digit:]]", f_focus_measurement_sold_2), 
    extract_numeric(f_focus_measurement_sold_2), 
    ifelse(f_focus_measurement_sold_2 %in% c("KG","kg","per kg","per kgs","kilo","kgs","kilogram", "kilograms kg"), 1, 0))) %>%
  mutate(cal_focus_measurement_sold = ifelse(f_focus_measurement_sold_2 == "per piece", 1, cal_focus_measurement_sold)) %>%
  mutate(cal_focus_measurement_sold = ifelse(f_focus_measurement_sold_2 %in% c("i dont have mangoes to sell", "does not sell",
                                                                               "i dont know", "mangoes not ready to be sold",
                                                                               "size of the bananas determine the price"),NA,cal_focus_measurement_sold)) %>%
  ...
  mutate(cal_focus_measurement_lost = ifelse(f_focus_measurement_lost == "per piece", 1, cal_focus_measurement_lost))
...
Data <- Data %>%  
  ...
  mutate(cal_focus_measurement_own_consumption = ifelse(f_focus_own_consumption_measurement == "per piece", 1, cal_focus_measurement_own_consumption)) 

##  ---- 2) Prep: Calculate quantities  ----
##Calculate the quantities
Data <- Data %>%
  mutate(cal_focus_quant_prod_kg = ifelse(f_focus_measurement_prod != "per piece", f_focus_quant_prod * cal_focus_measurement_prod, 0)) %>%
  mutate(cal_focus_quant_prod_per_piece = ifelse(f_focus_measurement_prod == "per piece", f_focus_quant_prod * cal_focus_measurement_prod, 0)) %>%
  ...
  mutate(cal_focus_quant_lost_kg = ifelse(f_focus_measurement_lost != "per piece", f_focus_quant_lost * cal_focus_measurement_lost, 0)) %>%
  mutate(cal_focus_quant_lost_per_piece = ifelse(f_focus_measurement_lost == "per piece", f_focus_quant_lost * cal_focus_measurement_lost, 0)) %>%
  
  mutate(cal_focus_quant_own_consumption_kg = ifelse(f_focus_own_consumption_measurement != "per piece", f_focus_own_consumption * cal_focus_measurement_own_consumption, 0)) %>%
  mutate(cal_focus_quant_own_consumption_per_piece = ifelse(f_focus_own_consumption_measurement == "per piece", f_focus_own_consumption * cal_focus_measurement_own_consumption, 0)) %>%
...
Data <- Data %>%
  mutate(cal_focus_revenue = cal_focus_quant_sold_kg * cal_focus_price_kg +  cal_focus_quant_sold_per_piece * cal_focus_price_per_piece)
  #Delete if type of coffee is not used in survey

Data <- Data %>% select(-f_focus_measurement_sold_2) 
...
Data <- Data %>%
  ...
  mutate(cal_focus_quant_own_consumption_per_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_own_consumption_per_piece )) %>%
  mutate(cal_focus_quant_lost_per_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_lost_per_piece)) %>%
  ...
  mutate(cal_focus_quant_own_consumption_per_piece =  sum(cal_focus_quant_own_consumption_per_piece)) %>%
  mutate(cal_focus_quant_lost_per_piece =  sum(cal_focus_quant_lost_per_piece)) %>%
  ...
  mutate(cal_focus_quant_own_consumption_per_piece = ifelse(`repeat no` >1  , NA, cal_focus_quant_own_consumption_per_piece )) %>%
  mutate(cal_focus_quant_lost_per_piece = ifelse(`repeat no` >1 , NA, cal_focus_quant_lost_per_piece)) %>%
...
subset <- Data %>% select(cal_focus_price_kg, cal_focus_price_per_piece)
##  ---- 3c) Calc: Productivity  ----  
##Calculate productivity
Data <- Data %>%
  mutate(cal_focus_productivity_kg_acre = ifelse(f_focus_measurement_prod != "per piece", cal_focus_quant_prod_kg/f_focus_crop_size_acre, NA)) %>%
#If crop is perennial crop, calculate yield/tree
  mutate(cal_focus_productivity_kg_tree = ifelse(f_focus_measurement_prod != "per piece",f_productivity_pertree, NA)) %>%
  # ALSO PRODUCTIVITY PER PIECE 
  mutate(cal_focus_productivity_per_piece_acre = ifelse(f_focus_measurement_prod == "per piece", cal_focus_quant_prod_per_piece/f_focus_crop_size_acre, NA))%>%
  mutate(cal_focus_productivity_per_piece_tree = ifelse(f_focus_measurement_prod == "per piece", f_productivity_pertree, NA))
...
Data <- Data %>%
  mutate(f_labour_otheractivity_rememberwage_othertype = 0,
         f_labour_cropmaint_paymentperactivity = 0,
...
write.xlsx(pi_info, file="pi_2022_goshen_mango.xlsx")
...
productivity <- Data %>%
  ...
  select(identifier,
         ...
         cal_focus_productivity_kg_tree,
         cal_focus_productivity_per_piece_acre,
         cal_focus_productivity_per_piece_tree,
         f_focus_quant_sold,
...
write.xlsx (productivity, file = "productivity_check_check.xlsx", overwrite = TRUE)
