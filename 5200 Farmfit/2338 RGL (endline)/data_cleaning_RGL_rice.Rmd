---
title: "data_cleaning_RGL_rice"
author: "Jildemarie Brouwer & Halimata Sawadogo"
date: "2023-08-14"
...
pi_filename <- "pi_2023_RGL_rice.xlsx"
data_filename <- "Input/515031340_RGL_rice_raw_data.xlsx"
survey_filename <- "Input/survey_RGL_rice.xlsx"
data_delivery <- "2023_rgl_anom_rice.xlsx"
...
baseline <- read_excel("Raw data from baseline with PI/24092021_rgl_rice_raw_data.xlsx")
baseline <- baseline %>% select(Identifier, pi_location_cascade_1_Mkoa, pi_location_cascade_1_Wilaya, name_of_farmer, mobile_number_farmer, hh_farmer_gender)
baseline <- baseline %>%
  distinct(Identifier, .keep_all = TRUE)
names(baseline)[names(baseline) == "pi_location_cascade_1_Mkoa"] <- "pi_location_cascade_1_Region"
names(baseline)[names(baseline) == "pi_location_cascade_1_Wilaya"] <- "pi_location_cascade_1_District"
...
Data <- Data %>%
  ...
  mutate(f_size_othermaincrop_2_hectare = ifelse(f_unit_land == "hectares", f_size_othermaincrop_2_acre, f_size_othermaincrop_2_acre / 2.471)) %>%
...
Data <- Data %>%
  ...
  mutate(f_focus_measurement_prod = ifelse(identifier == "y461faeapvj9", "100 kg bag  gunia 100 kg", f_focus_measurement_prod)) %>%
 #quantity sold>prod...> set equal to 20
  mutate(f_focus_quant_prod = ifelse(identifier == "1eytj9hpudvq", 20, f_focus_quant_prod))
```
...
```{r}
Data <- Data %>%
   # value quantity sold is 0 with --> set unit to NA
  mutate(
    f_focus_measurement_sold = ifelse(f_focus_quant_sold == 0, NA_character_, f_focus_measurement_sold)
  )
#Data <- Data %>%
#  mutate(f_focus_measurement_sold = case_when(
#    identifier %in% c("3pdefqg7pmb3", "yk1chcw1qv27", "40hak5cf3p0j",
#                                                        "0xnf42ueu5qs", "gjf58qprnsw1", "vjpns0sd4058",
#                                                        "8et4xacxp51j", "hmpqek07j69j", "q8nn4svmay50",
#                                                        "ucfvjrb5gg6b") ~ NA_character_ ,
...
```{r}
Data <- Data %>%
  mutate(f_focus_own_consumption_measurement = ifelse(identifier == "1cedwu746qg1", NA_character_, f_focus_own_consumption_measurement))
```
...
Data <- Data %>%
  mutate(f_labour_nurserymaint_paymentperactivity = 0,
         ...
         f_labour_landprep_nrhours = 0,
         f_labour_nurserymaint_nrhours = 0,
         f_labour_agrochemicalapp_nrhours=0,
         f_labour_irrigation_nrhours=0,
         f_labour_fertilizerapp_nrhours = 0,
         f_labour_compostprep_nrhours = 0,
         f_labour_harvesting_nrhours = 0,
         f_labour_postharvest_nrhours = 0)
...
write.xlsx(pi_info, file="pi_2023_RGL_rice.xlsx")
...
write.xlsx (productivity, file = "productivity_check_rgl_rice.xlsx", overwrite = TRUE)
```
