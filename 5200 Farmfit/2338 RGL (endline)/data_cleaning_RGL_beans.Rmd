---
title: "data_cleaning_RGL_beans"
authors: "Maurits Kruisheer & Halimata Sawadogo & Jildemarie Brouwer"
date: "2023-08-14"
...
pi_filename <- "pi_2023_RGL_beans.xlsx"
data_filename <- "Input/515031340_RGL_beans_raw_data.xlsx"
survey_filename <- "Input/survey_RGL_beans.xlsx"
data_delivery <- "2023_rgl_anom_beans.xlsx"
...
Data <- Data %>%
  rename(f_focus_measurement_lost = f_rice_measurement_lost,
         f_focus_lost_yn = f_rice_lost_yn,
         f_focus_quant_lost = f_rice_quant_lost,
         `f_focus_measurement_lost--OTHER--` = `f_rice_measurement_lost--OTHER--`) 
```
...
baseline <- read_excel("Raw data from baseline with PI/24092021_rgl_beans_raw_data.xlsx")
baseline <- baseline %>% select(Identifier, pi_location_cascade_1_Region, pi_location_cascade_1_District, name_of_farmer, mobile_number_farmer, hh_farmer_gender)
baseline <- baseline %>%
...
result <- baseline %>%
  ...
  group_by(pi_location_cascade_Level_1) %>%
...
Data <- Data %>%
  ...
  mutate_at(vars(numerical_columns), funs(as.numeric)) %>%
    ...
    #General case
  mutate(f_number_of_crops = as.numeric(f_number_of_crops)) %>%  
  mutate(f_focus_harvest_num = as.numeric(f_focus_harvest_num)) %>%
...
```
```{r}
# change units of land where they are wrongly reported 
wrong_identifiers <- c("r21ye1wwdvtt", "v3t0mn4kyswa", "bwervnxagexm", "cm3ag1qs4u1e", "9qsjb31wgnw2", "axnxmr4bvjyd", "bwt2vxjqh3hv", "4vetkmqwh8jq", "5y62fr0q9g56", "t039wj3wbw79", "rmk66rx2p3a1", "tk897h987nts","u33uj40qkq3b", "8678jwqx6m00", "g26q99b5c1ww", "xxckbau3gm0j", "cphusyvmapkm")

Data$f_unit_land <- ifelse(Data$identifier %in% wrong_identifiers, "acres", Data$f_unit_land)
```
...
# One farmer indicated a measurement unit of poles, but this should be hectares
Data$f_unit_land <- ifelse(Data$f_unit_land == "poles", "hectares", Data$f_unit_land)
...
Data <- Data %>%
  ...
  mutate(f_size_hectare = ifelse(f_unit_land == "hectares", f_size, f_size / 2.471)) %>%
  mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "hectares", f_focus_crop_size, f_focus_crop_size / 2.471)) %>%
  mutate(f_size_othermaincrop_1_hectare = ifelse(f_unit_land == "hectares", f_size_othermaincrop_1, f_size_othermaincrop_1 / 2.471)) %>%
  mutate(f_size_othermaincrop_2_hectare = ifelse(f_unit_land == "hectares", f_size_othermaincrop_2_acre, f_size_othermaincrop_2 / 2.471)) %>%
...
Cleaning f_focus_price

```{r, include = FALSE}
# Calculate median prices for specific measurements
median_price_100kg <- Data %>%
  filter(f_focus_measurement_sold == "100 kg bag gunia 100 kg") %>%
  summarize(median_price_100kg = median(f_focus_price, na.rm = TRUE))

median_price_20kg <- Data %>%
  filter(f_focus_measurement_sold == "a bucket 20kgs") %>%
  summarize(median_price_20kg = median(f_focus_price, na.rm = TRUE))

# Replace extreme prices with missing values
Data <- Data %>%
  mutate(f_focus_price = ifelse(identifier == "p9mfhq6k5f2y", NA, f_focus_price)) %>%
  
  # Replace prices with median values
  mutate(f_focus_price = ifelse(identifier == "bpahg3qaec5w", ifelse(is.na(median_price_20kg$median_price_20kg), f_focus_price, median_price_20kg$median_price_20kg), f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "byppa6ska5mg", ifelse(is.na(median_price_100kg$median_price_100kg), f_focus_price, median_price_100kg$median_price_100kg), f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "8678jwqx6m00" | identifier == "r21ye1wwdvtt",f_focus_price * 100, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "g26q99b5c1ww",median_price_20kg$median_price_20kg, f_focus_price))

```

clean f_focus_measurement_prod

```{r}
Data <- Data %>%
  # missing value f_focus_measurement_prod , replace by 100 kg bag  gunia 100 kg
  mutate(f_focus_measurement_prod = ifelse(identifier == "q5akxqsea9wh", "100 kg bag  gunia 100 kg", f_focus_measurement_prod)) %>%
 # missing value f_focus_measurement_prod , replace by a bucket 20kgs
    mutate(f_focus_measurement_prod = ifelse(identifier == "0gthmpyenbvm", "a bucket 20kgs", f_focus_measurement_prod))
```

clean f_focus_quant_sold

```{r}
 Data <- Data %>%
  # Outlier quantity sold --> set equal to missing
  mutate(f_focus_quant_sold = ifelse(identifier == "8678jwqx6m00", NA, f_focus_quant_sold)) %>%
  mutate(f_focus_quant_sold = ifelse(identifier == "7y0sk18gya1h", NA, f_focus_quant_sold)) %>%
   # Outlier quantity sold --> set equal to quantity produced
  mutate(f_focus_quant_sold = ifelse(identifier == "g26q99b5c1ww", 250, f_focus_quant_sold))
  
```

clean f_focus_own_consumption_measurement

```{r}
Data <- Data %>%
  mutate(f_focus_own_consumption_measurement = case_when(
    identifier %in% c("aeu678bfwjmx", "1hwuqqsx2meu") ~ NA_character_,
    TRUE ~ f_focus_own_consumption_measurement
  ))

```

f_labour_hired_share and su_farmer_organisation_sell_share

```{r}
#cleaning multiple entry by replacing with the first value
Data <- Data %>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="1kuchw81mbyc", 6, su_farmer_organisation_sell_share))%>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="ahhxbud43tc1", 6, su_farmer_organisation_sell_share))%>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="21venw1qxd2v", 6, su_farmer_organisation_sell_share))%>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="y36sfardu2xu", 5, su_farmer_organisation_sell_share))%>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="kqm8wcynmhx0", 6, su_farmer_organisation_sell_share))%>%
  mutate(su_farmer_organisation_sell_share = ifelse(identifier=="sq4km03cha98", 6, su_farmer_organisation_sell_share))%>%
#set to numeric
mutate(f_labour_hired_share=as.numeric(f_labour_hired_share))%>%
mutate(su_farmer_organisation_sell_share=as.numeric(su_farmer_organisation_sell_share))
...
```{r}
# Only run this chunck of code only AFTER you did the productivity check! 
...
```{r}
#able(Data$f_focus_measurement_prod)
...
##Measurement unit for production
    mutate(f_focus_measurement_prod = ifelse(f_focus_measurement_prod == "100 kg bag  gunia 100 kg", "100 kg bag", f_focus_measurement_prod)) %>%
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      extract_numeric(f_focus_measurement_prod), 
      ifelse(f_focus_measurement_prod %in% c("kgs"), 1, 0))) 
...
Data <- Data %>%
  ## Measurement unit for sold
  mutate(f_focus_measurement_sold = ifelse(f_focus_measurement_sold == "100 kg bag gunia 100 kg", "100 kg bag", f_focus_measurement_sold)) %>%
  mutate(cal_focus_measurement_sold := ifelse(grepl("[[:digit:]]", f_focus_measurement_sold),
    extract_numeric(f_focus_measurement_sold),
    ifelse(f_focus_measurement_sold %in% c("kgs"), 1, 0)))
...
# measurement unit for lost
    mutate(f_focus_measurement_lost = ifelse(f_focus_measurement_lost == "100 kg bag  gunia 100 kg", "100 kg bag", f_focus_measurement_lost)) %>%
  mutate(cal_focus_measurement_lost := ifelse(
    grepl("[[:digit:]]", f_focus_measurement_lost), 
    extract_numeric(f_focus_measurement_lost), 
    ifelse(f_focus_measurement_lost %in% c("kgs"), 1, 0))) 
...
Data <- Data %>%
# measurement unit for lost
    mutate(f_focus_own_consumption_measurement = ifelse(f_focus_own_consumption_measurement == "100 kg bag  gunia 100 kg", "100 kg bag", f_focus_own_consumption_measurement))%>% 
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in% c("kgs"), 1, 0))) 
```
...
```{r}
# Replace NA values of these cost variables by 0
...
Make the sum of labour, inputs and transportation cost.

```{r, include=FALSE}
Data <- Data %>%
    mutate(cal_focus_cost = cal_labour_cost + cal_inputs_costs + f_transport+f_labour_othercosts) 
```

### 8) Net-income focus crop

Net income focus crop is calculated by revenue - cost.
...
```{r}
# there was an issue with the survey, variable f_othermaincrop_2_costs_transport needs to be deleted
Data <- Data %>% select(-f_othermaincrop_2_costs_transport)
```

```{r, include = FALSE}
# Calculate total cost for other main crop 1
Data <- Data %>%
  mutate(cal_othermaincrop_1_cost = rowSums(select(., starts_with("f_othermaincrop_1_cost")), na.rm = TRUE))

# Calculate total cost for other main crop 2
Data <- Data %>%
  mutate(cal_othermaincrop_2_cost = rowSums(select(., starts_with("f_othermaincrop_2_cost")), na.rm = TRUE))

Data <- Data %>%
  # Replace missing values in other main crop 1 income sold with 0
  mutate(cal_othermaincrop_1_inc_sold = ifelse(is.na(f_othermaincrop_1_inc_sold), 0, f_othermaincrop_1_inc_sold)) %>%
  # Replace missing values in other main crop 2 income sold with 0
  mutate(cal_othermaincrop_2_inc_sold = ifelse(is.na(f_othermaincrop_2_inc_sold), 0, f_othermaincrop_2_inc_sold))  %>%
  # Calculate total income for other main crops
  ...
  # Replace missing values in other crop income with 0
  mutate(cal_other_crop_income = ifelse(is.na(f_other_crop_income), 0, f_other_crop_income)) %>%
  
   # Calculate total income for other crops (including main crops)
  mutate(cal_other_crop_income = cal_othermaincrop_income + cal_other_crop_income) %>%
  # Remove the intermediate column used for calculation (cal_othermaincrop_income)
...
#Approach 1
 ...
 #Input costs
    ...
    Data$cal_livestock_income[is.na(Data$cal_livestock_income)] <- 0

```

```{r}
# replace missing values cal_livestock income with 0
Data$cal_livestock_income <- Data$cal_livestock_income %>% replace(is.na(.), 0)
...
# GENERAL ON FARM COSTS
  mutate(cal_farm_other_costs = Data %>% 
           select(f_costs_land,
             ...
             apply(., 1, sum, na.rm=TRUE)) %>%
   mutate(cal_farm_costs_general = cal_equipment_costs + cal_farm_other_costs)
```
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(cal_off_farm_labour_income = ifelse(is.na(f_nonfarm_enterpr_1_profit), 0, f_nonfarm_enterpr_1_profit) +
                                      ifelse(is.na(f_nonfarm_enterpr_2_profit), 0, f_nonfarm_enterpr_2_profit) +
                                      ifelse(is.na(f_nonfarm_enterpr_3_profit), 0, f_nonfarm_enterpr_3_profit) +
                                      ifelse(is.na(f_income_offfarmlabour_wage), 0, f_income_offfarmlabour_wage))
```
...
Data <- Data %>%
  mutate(
    cal_farm_income = cal_focus_income + cal_other_crop_income + cal_livestock_income - 
                      cal_equipment_costs - cal_farm_other_costs,
    cal_actual_income = cal_farm_income + cal_off_farm_labour_income + cal_offfarm_non_labour_income,
    cal_farm_income = ifelse(is.na(farmer_present), NA, cal_farm_income),
    cal_actual_income = ifelse(is.na(farmer_present), NA, cal_actual_income)
  )
...
for (j in multiple_mc) {
  ...
  if (j == "hh_loan_one_other_purpose") {
    next
  }
...
```{r}
baseline_farmers <- read_excel("pi_rgl_baseline.xlsx")
...
colnames(Data)[colnames(Data) == "Identifier"] <- "cal_match_baseline"
...
write.xlsx(pi_info, file="pi_2023_RGL_beans.xlsx")
...
productivity <- Data %>%
    ...
    select(identifier,
           ...
           su_farmer_organisation_sell_share)
  
write.xlsx (productivity, file = "productivity_check_rgl_beans.xlsx", overwrite = TRUE)
```
