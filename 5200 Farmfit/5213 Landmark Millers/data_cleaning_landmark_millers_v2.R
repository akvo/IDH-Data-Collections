...
setwd("/Users/ineslesire/Documents/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/5213 Landmark Millers")
##  ---- Describe datafiles ----
##DOWNLOAD THESE ALL TOGETHER IN A FOLDER
pi_filename <- "pi_2022_lmm_maize_sorghum.xlsx"
data_filename <- "23052022_lmm_maize_sorghum_raw_data.xlsx"
survey_filename <- "survey_lmm_maize_sorghum.xlsx"
data_delivery <- "2022_lmm_anom_maize_sorghum.xlsx"
...
Data <- Data %>%
  ...
  mutate(f_size_othermaincrop_2_acre = ifelse(f_unit_land == "hectares", 
    ...
    mutate(f_size_othermaincrop_3_hectare = f_size_othermaincrop_3_acre / 2.471) %>%
...
##  ---- _______ ----
##  ---- 3) BY LOCATION ----
# Location descriptive for aggregation
location_var <- Data %>% 
  select(pi_location_cascade_first_level) %>% 
  filter(!is.na(pi_location_cascade_first_level)) %>%
  table() %>% 
  melt("pi_location_cascade_first_level", value.name="n")

## ---- a) numerical descriptives ----

numerical_descriptives_by_location <- Data %>%
  ...
  left_join(survey_questions) %>%
  left_join(location_var) %>%
  select("variable","pi_location_cascade_first_level","n", "freq", "mean","sd", "min","max") 
...
single_mc <- survey_questions %>% 
  filter(type=="option") %>% 
  filter(is.na(multiple)) %>% 
  filter(variable != "pi_location_cascade_first_level") %>%
  pull(variable)

single_categorical_descriptives_by_location <- location_var
...
for(i in single_mc){
  ...
  if(any(names(Data) %in% i)){
    ...
    frequencies <- Data %>% 
      select(pi_location_cascade_first_level, all_of(i)) %>% 
      table() %>% 
      melt(c("pi_location_cascade_first_level","category"), value.name="freq") %>%
      mutate("variable" = i) %>%
      left_join(location_var) %>%
      mutate("%" = round(freq/n,2)*100) %>%
      mutate(category = as.character(category))
    
    single_categorical_descriptives_by_location <- single_categorical_descriptives_by_location %>% 
      full_join(frequencies) %>%
      select(variable, pi_location_cascade_first_level, n, category, freq, "%")
  }
}
...
multiple_categorical_descriptives_by_location <- location_var
...
for(j in multiple_mc){
  ...
  if(any(names(Data) %in% j)){
    ...
    frequencies <- Data %>% 
      select(pi_location_cascade_first_level, all_of(j)) %>% 
      cSplit(j, "|") %>%
      gather(key, value, -pi_location_cascade_first_level) %>%
      select(-key) %>% table() %>%
      melt(c("pi_location_cascade_first_level","category"), value.name="freq") %>%
      mutate("variable" = j) %>%
      left_join(location_var) %>%
    ...
    multiple_categorical_descriptives_by_location <- multiple_categorical_descriptives_by_location %>% 
      full_join(frequencies) %>%
      select(variable, pi_location_cascade_first_level, n, category, freq, "%")
  }
}
...
data <- data.frame(variable) %>%
  ...
  sets <- list(
    ...
    "Num. desc. by location" = numerical_descriptives_by_location,
    "Cat. desc. single by location" = single_categorical_descriptives_by_location,
    "Cat. desc. multi by location" = multiple_categorical_descriptives_by_location,
  ...
  write.xlsx(data, file = "check.xlsx")
  ...
  productivity <- Data %>%
    ...
    select(identifier,
           submitter,
           `submission date`,
           starts_with("pi_location_cascade_level"),
           f_maize_quant_prod,
