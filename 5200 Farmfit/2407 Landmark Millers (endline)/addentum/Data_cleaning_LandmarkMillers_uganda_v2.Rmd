...
```{r}
data_filename <- "DA-627930161_2407 Landmark Millers (endline).xlsx"
survey_filename <- "SF-627930161_2407 Landmark Millers (endline)_merged.xlsx"
data_additional_name <- "DATA_ANALYSIS-Landmark millers - Endline questions-Maize and Sorghum.xlsx"
...
data_delivery <- paste(year,"_",case,"_anom_",sdm_crop,"_endline_v2",".xlsx", sep = "")
```
...
#additonal data for merging
data_endline_questions <- read_excel(data_additional_name)
```


# Merge data
Clean endline questions
```{r, include = FALSE}
# rename phone number mistakenly writen as focus_crop
data_endline_questions <- data_endline_questions %>%
  rename(mobile_number_farmer = focus_crop)

data_endline_questions <- data_endline_questions %>%
  mutate(mobile_number_farmer = as.character(mobile_number_farmer))
# Add leading zeros to phone numbers that have less than 10 digits
data_endline_questions$mobile_number_farmer <- str_pad(data_endline_questions$mobile_number_farmer, width = 10, side = "left", pad = "0")

#remove unecessary questions
data_endline_questions <- data_endline_questions %>%
  select(-Identifier, -`Repeat no`, -`Display Name`, -`Device identifier`, -Instance, -`Submission Date`, -Submitter, -Duration, -`Form version`, -ic_informed_consent, -farmer_name)

```

merge based on phone number
```{r, include = FALSE}
# Merging Data with data_endline_questions using mobile_number_farmer as the key
Data <- Data %>%
  left_join(data_endline_questions, by = "mobile_number_farmer")
...
Data <- Data %>%
  ...
  mutate_at(vars(numerical_columns), funs(as.numeric)) %>%
    ...
    mutate(f_sorghum_harvest_num = as.numeric(f_sorghum_harvest_num)) %>%
...
Data <- Data %>%
  mutate(f_size = ifelse(identifier %in% c("3qdbgeh1mmdv", "16k0hfg443er"), f_maize_crop_size + f_sorghum_crop_size, f_size))

Data <- Data %>%
  mutate(f_size = ifelse(f_size == 9998 | f_size == 9999, NA, 
                         ifelse(f_size > 2000 & f_unit_land == "acres", f_size / 100,
                         ifelse(f_unit_land == "acres" & f_size >=400, f_size / 10 , f_size))),
         f_maize_crop_size = ifelse(f_maize_crop_size == 9998 | f_maize_crop_size == 9999, NA,
                            ifelse(f_maize_crop_size >= 100 & f_unit_land == "acres", f_maize_crop_size / 100, f_maize_crop_size)),
         f_sorghum_crop_size = ifelse(f_sorghum_crop_size == 9998 | f_sorghum_crop_size == 9999, NA,
                      ifelse(f_sorghum_crop_size >= 100 & f_unit_land == "acres", f_sorghum_crop_size / 100, f_sorghum_crop_size)))
...
#        na.rm = TRUE, remove = FALSE, sep = " ") %>%
  ...
  # Convert maize crop size to acres
  mutate(f_maize_crop_size_acre = case_when(
    f_unit_land == "hectares" ~ f_maize_crop_size * 2.471,
    f_unit_land == "poles" ~ f_maize_crop_size / 160,
    f_unit_land == "squared meters" ~ f_maize_crop_size / 4047,
    TRUE ~ f_maize_crop_size
  )) %>%
  
  # Convert maize crop size to hectares
  mutate(f_maize_crop_size_hectare = case_when(
    f_unit_land == "hectares" ~ f_maize_crop_size,
    f_unit_land == "poles" ~ f_maize_crop_size / 395.4,
    f_unit_land == "squared meters" ~ f_maize_crop_size / 10000,
    TRUE ~ f_maize_crop_size_acre / 2.471
  )) %>%
  
  # Fill missing values in Maize crop size
#  mutate(f_maize_crop_size_acre = na.locf(f_maize_crop_size_acre, na.rm = FALSE)) %>%
    
  
  # Convert sorghom crop size to acres
  mutate(f_sorghum_crop_size_acre = case_when(
    f_unit_land == "hectares" ~ f_sorghum_crop_size * 2.471,
    f_unit_land == "poles" ~ f_sorghum_crop_size / 160,
    f_unit_land == "squared meters" ~ f_sorghum_crop_size / 4047,
    TRUE ~ f_sorghum_crop_size
  )) %>%
  
  # Convert sorghum crop size to hectares
  mutate(f_sorghum_crop_size_hectare = case_when(
    f_unit_land == "hectares" ~ f_sorghum_crop_size,
    f_unit_land == "poles" ~ f_sorghum_crop_size / 395.4,
    f_unit_land == "squared meters" ~ f_sorghum_crop_size / 10000,
    TRUE ~ f_sorghum_crop_size_acre / 2.471
  )) 
#%>%
    
  # Fill missing values in Sorghum crop size
...
#*******************************************************************************
# SKIP UNTIL YOU SEE END OF EXECUTION SKIPPING
#*******************************************************************************
...
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#quantities too much regarding crop farm size (2 acres)
Data <- Data %>%
  mutate(f_sorghum_quant_prod = ifelse(identifier== "16k0hfg443er", f_sorghum_quant_prod / 10, f_sorghum_quant_prod),
         f_sorghum_quant_sold = ifelse(identifier== "16k0hfg443er", f_sorghum_quant_sold / 10, f_sorghum_quant_sold),
         f_sorghum_measurement_prod = ifelse(identifier== "16k0hfg443er", "100 kg bags", f_sorghum_measurement_prod))%>%
  
#production lower than sold. check farm size and quantities and keep the one that is correct. before that, replace 9998 and 9999 to missing
mutate(f_sorghum_quant_prod = ifelse(f_sorghum_quant_prod == 9998 | f_sorghum_quant_prod == 9999, NA, f_sorghum_quant_prod),
       f_sorghum_quant_sold = ifelse(f_sorghum_quant_sold == 9998 | f_sorghum_quant_sold == 9999, NA, f_sorghum_quant_sold)) %>%
  #note replace prod by sold when it's lower than sold
mutate(f_sorghum_quant_prod = ifelse(f_sorghum_quant_prod < f_sorghum_quant_sold, f_sorghum_quant_sold, f_sorghum_quant_prod))


#some prices are not in line with the unit sold. regarding farm size, some units should be in 100 kg bags
Data <- Data %>%
  mutate(f_sorghum_quant_prod = ifelse(identifier == "bydg8pm4a6cb", f_sorghum_quant_prod * 100, f_sorghum_quant_prod),
         f_sorghum_quant_sold = ifelse(identifier == "bydg8pm4a6cb", f_sorghum_quant_sold * 100, f_sorghum_quant_sold))
```

#### Maize quantities
```{r}
#error in maize quantity. sould be less than value entered regarding the farm size and quantity sold
Data <- Data %>%
  mutate(f_maize_quant_prod = ifelse(identifier== "rw3sybpkbd20", f_maize_quant_prod / 10, f_maize_quant_prod))%>%
#production lower than sold. check farm size and quantities and keep the one that is correct. before that, replace 9998 and 9999 to missing
mutate(f_maize_quant_prod = ifelse(f_maize_quant_prod == 9998 | f_maize_quant_prod == 9999, NA, f_maize_quant_prod),
       f_maize_quant_sold = ifelse(f_maize_quant_sold == 9998 | f_maize_quant_sold == 9999, NA, f_maize_quant_sold)) %>%
  #note replace prod by sold when it's lower than sold
mutate(f_maize_quant_prod = ifelse(f_maize_quant_prod < f_maize_quant_sold, f_maize_quant_sold, f_maize_quant_prod))

#some prices are not in line with the unit sold. regarding farm size, some units should be in kg instead of 50 kg bags. 
Data <- Data %>%
  mutate(f_maize_quant_prod = ifelse(identifier == "3s008awkqkfx", f_maize_quant_prod * 50, f_maize_quant_prod),
         f_maize_quant_sold = ifelse(identifier == "3s008awkqkfx", f_maize_quant_sold * 50, f_maize_quant_sold))
```


#### Maize measurement units

```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#price given per kgs, not in line with quantities. replace unit by kgs
Data <- Data %>%
  mutate(f_maize_measurement_sold = ifelse(identifier== "kqgejb1fv9a3", "kgs", f_maize_measurement_sold),
         f_maize_measurement_prod = ifelse(identifier== "kqgejb1fv9a3", "kgs", f_maize_measurement_prod))

# convert other units in to kgs to protect from outlier function. only few farmers are concerned. also check wether price is per unit then convert to kgs price. divid price by unit coefficient
unit_replace_50kg <- "50 kg bags"
unit_replace_100kg <- "100 kg bags"
Data <- Data %>%
  mutate(f_maize_quant_sold = 
           ifelse(f_maize_measurement_sold == unit_replace_50kg, f_maize_quant_sold * 50,
                  ifelse(f_maize_measurement_sold == unit_replace_100kg, f_maize_quant_sold * 100,
                         f_maize_quant_sold)),
           f_maize_price = 
           ifelse(f_maize_measurement_sold == unit_replace_50kg, f_maize_price / 50,
                  ifelse(f_maize_measurement_sold == unit_replace_100kg, f_maize_price / 100,
                         f_maize_price)),
    f_maize_measurement_sold = 
      ifelse(f_maize_measurement_sold == unit_replace_100kg |f_maize_measurement_sold == unit_replace_50kg , "kgs", 
             f_maize_measurement_sold),
    
         f_maize_quant_prod = 
      ifelse(f_maize_measurement_prod == unit_replace_50kg, f_maize_quant_prod * 50,
             ifelse(f_maize_measurement_prod == unit_replace_100kg, f_maize_quant_prod * 100,
             f_maize_quant_prod)),
             
         f_maize_measurement_prod = 
           ifelse(f_maize_measurement_prod == unit_replace_50kg |f_maize_measurement_prod == unit_replace_100kg, "kgs", 
                  f_maize_measurement_prod)) %>%
      
#Do the same for consumption 
  mutate(f_maize_own_consumption = 
           ifelse(f_maize_own_consumption_measurement == unit_replace_100kg, f_maize_own_consumption * 100, 
                  f_maize_own_consumption),
         
    f_maize_own_consumption_measurement = 
      ifelse(f_maize_own_consumption_measurement == unit_replace_100kg, "kgs", 
             f_maize_own_consumption_measurement))

```


#### Sorghum measurement units
```{r}
#Sorghum quantity in 100 kg bags but price is given per kgs. then convert quant to kgs or price according to unit (check farm size also). also harmonise others in kgs in order to protect from outlier function
# identifier = kqgejb1fv9a3 , quantity prod ok, change only unit to kgs
# identifier = kqgejb1fv9a3 , quantity * 100, and change only unit to kgs
Data <- Data %>%
  mutate(f_sorghum_measurement_sold = ifelse(identifier %in% c("kqgejb1fv9a3", "pxqs7befhrh0"), "kgs", f_sorghum_measurement_sold),
         f_sorghum_measurement_prod = ifelse(identifier %in% c("kqgejb1fv9a3", "pxqs7befhrh0", "8by81evsxqnj"), "kgs", f_sorghum_measurement_prod),
         f_sorghum_quant_prod = ifelse(identifier == "pxqs7befhrh0", f_sorghum_quant_prod * 100, f_sorghum_quant_prod),
         f_sorghum_quant_sold = ifelse(identifier == "pxqs7befhrh0", f_sorghum_quant_sold * 100, f_sorghum_quant_sold))


#harmonize into kgs and change price into price/100
unit_replace <- "100 kg bags"
Data <- Data %>%
  mutate(f_sorghum_quant_sold = 
           ifelse(f_sorghum_measurement_sold == unit_replace, f_sorghum_quant_sold * 100, 
                  f_sorghum_quant_sold),
         
    f_sorghum_measurement_sold = 
      ifelse(f_sorghum_measurement_sold == unit_replace, "kgs", 
             f_sorghum_measurement_sold),
    
    f_sorghum_price = 
      ifelse(f_sorghum_measurement_sold == unit_replace, f_sorghum_price / 100, 
             f_sorghum_price),
    
    f_sorghum_quant_prod = 
      ifelse(f_sorghum_measurement_prod == unit_replace, f_sorghum_quant_prod * 100, 
             f_sorghum_quant_prod),
    
         f_sorghum_measurement_prod = 
      ifelse(f_sorghum_measurement_prod == unit_replace, "kgs", 
             f_sorghum_measurement_prod)) %>%
#Do the same for consumption and lost
  mutate(f_sorghum_own_consumption = 
           ifelse(f_sorghum_own_consumption_measurement == unit_replace, f_sorghum_own_consumption * 100, 
                  f_sorghum_own_consumption),
    f_sorghum_own_consumption_measurement = 
      ifelse(f_sorghum_own_consumption_measurement == unit_replace, "kgs", 
             f_sorghum_own_consumption_measurement))
...
#### fill unit of 0 quantity to missing
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
Data <- Data %>%
  mutate(f_maize_measurement_sold = ifelse(f_maize_quant_sold==0, NA_character_ , f_maize_measurement_sold),
         f_maize_measurement_prod = ifelse(f_maize_quant_prod==0, NA_character_ , f_maize_measurement_prod))

Data <- Data %>%
  mutate(f_sorghum_measurement_sold = ifelse(f_sorghum_quant_sold==0, NA_character_ , f_sorghum_measurement_sold),
         f_sorghum_measurement_prod = ifelse(f_sorghum_quant_prod==0, NA_character_ , f_sorghum_measurement_prod))
...
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#some prices for quantities in kgs are higher. replace by median price
maize_median_price_kgs<- Data %>%
  filter(f_sorghum_measurement_sold == "kgs") %>%
  summarize(maize_median_price_kgs = median(f_maize_price, na.rm = TRUE)) 

Data <- Data %>%
  mutate(f_maize_price=ifelse(identifier %in% c("wt1tggvau840", "jxbbfjeh8vyu", "cn90vaqhsn19", "uhj39n6r5pkn", "qqqg824v952c", "ns313ebe1cda", "ev73uw9y3344"), maize_median_price_kgs$maize_median_price_kgs, f_maize_price))

```


#### Adjustments for Sorghum price

```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#some prices for quantities in kgs are higher. replace by median price
sorghum_median_price_kgs<- Data %>%
  filter(f_sorghum_measurement_sold == "kgs") %>%
  summarize(sorghum_median_price_kgs = median(f_sorghum_price, na.rm = TRUE)) 

Data <- Data %>%
  mutate(f_sorghum_price=ifelse(identifier %in% c("naqyn3dpq2kd", "wt1tggvau840", "87fy0fw1hjv9"), sorghum_median_price_kgs$sorghum_median_price_kgs, f_sorghum_price))
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(f_maize_quant_prod= ifelse(identifier == "3s008awkqkfx", 100, 
                                     f_maize_quant_prod),
f_maize_quant_sold= ifelse(identifier == "n7jge3pv0e4b", 3000, 
                                     ifelse(identifier == "3s008awkqkfx", 50, 
                                     f_maize_quant_sold)),
         f_maize_price = ifelse(identifier == "3s008awkqkfx", 1100, f_maize_price),

f_sorghum_quant_sold= ifelse(identifier == "bydg8pm4a6cb", 600, f_sorghum_quant_sold),
f_sorghum_quant_prod= ifelse(identifier == "bydg8pm4a6cb", 600, f_sorghum_quant_prod),
f_sorghum_own_consumption= ifelse(identifier == "ech96j1s48fy", 100,
                                  ifelse(identifier == "8atktuy78g5p", 200,
                                         f_sorghum_own_consumption))
)
...
number_of_seasons_sorghun <- max(Data$f_sorghum_harvest_num, na.rm=TRUE)
```
...
table(Data$f_sorghum_measurement_prod)
table(Data$f_sorghum_measurement_sold)
```

To do so, we extract the numerical values from the measurement units for quantity produced, sold, consumed and lost. Always check by hand whether this is going well. We extract the numbers from the measurement units to calculate total quantities. If there are other measurement units, not related to kg, put a value for those in the variable: cal_maize_measurement_prod; cal_sorghum_measurement_prod

**Measurement unit production (adapt the following code with what appear in the previous output)**
...
##Measurement unit for production
  mutate(cal_maize_measurement_prod := ifelse(grepl("[[:digit:]]", f_maize_measurement_prod), 
      ...
      ifelse(f_maize_measurement_prod %in%c("kg","kgs"), 1, 0))) %>%

  mutate(cal_sorghum_measurement_prod := ifelse(grepl("[[:digit:]]", f_sorghum_measurement_prod), 
      extract_numeric(f_sorghum_measurement_prod), 
      ifelse(f_sorghum_measurement_prod %in%c("kg","kgs"), 1, 0)))
```
...
##Measurement unit for sales focus crops
  ...
  mutate(cal_sorghum_measurement_sold := ifelse(
      ...
      ifelse(f_sorghum_measurement_sold %in%c("kg","kgs"), 1, 0)))
```

**Measurement unit focus crops lost**
...
# measurement unit for lost
  mutate(cal_maize_lost_measurement := ifelse(
    ...
    ifelse(f_maize_measurement_lost %in%c("kg","kgs"), 1, 0))) %>%

  mutate(cal_sorghum_lost_measurement := ifelse(
    grepl("[[:digit:]]", f_sorghum_measurement_lost), 
    extract_numeric(f_sorghum_measurement_lost), 
    ifelse(f_sorghum_measurement_lost %in%c("kg","kgs"), 1, 0)))
```

**Measurement unit focus crops own consumption**
...
# measurement unit for lost
  mutate(cal_maize_measurement_own_consumption := ifelse(
    ...
    ifelse(f_maize_own_consumption_measurement %in%c("kg","kgs"), 1, 0))) %>%

  mutate(cal_sorghum_measurement_own_consumption := ifelse(
    grepl("[[:digit:]]", f_sorghum_own_consumption_measurement), 
    extract_numeric(f_sorghum_own_consumption_measurement), 
    ifelse(f_sorghum_own_consumption_measurement %in%c("kg","kgs"), 1, 0))) 
```
...
```{r, include = FALSE}
##Calculate the quantities focus crops
Data <- Data %>%
  ...
  mutate(cal_sorghum_quant_sold_kg = f_sorghum_quant_sold * cal_sorghum_measurement_sold) %>%
  mutate(cal_sorghum_quant_lost_kg = f_sorghum_quant_lost * cal_sorghum_lost_measurement) %>%
  mutate(cal_sorghum_quant_own_consumption_kg = f_sorghum_own_consumption * cal_sorghum_measurement_own_consumption)%>%
  mutate_at(vars(starts_with("cal_sorghum_quant")), ~replace_na(., 0)) 
...
Data <- Data %>%
  ...
  mutate(cal_maize_price = replace_na(cal_maize_price, 0)) %>%

  mutate(cal_sorghum_price = ifelse(cal_sorghum_measurement_prod != 0, f_sorghum_price/cal_sorghum_measurement_sold, f_sorghum_price),
    cal_sorghum_price = ifelse(is.na(cal_sorghum_quant_sold_kg) | cal_sorghum_quant_sold_kg == 0, NA, cal_sorghum_price))%>%
  mutate(cal_sorghum_price = replace_na(cal_sorghum_price, 0))

Data <- Data %>% 
  mutate(cal_sorghum_price = ifelse(cal_sorghum_quant_sold_kg >0 & is.na(cal_sorghum_price), median(cal_sorghum_price), cal_sorghum_price)) %>%
...
maize_quantities_lost_subset <- Data %>% select(f_maize_measurement_lost, cal_maize_lost_measurement, f_maize_quant_lost, cal_maize_quant_lost_kg)

maize_quantities_own_consumption_subset <- Data %>% select(f_maize_own_consumption_measurement, cal_maize_measurement_own_consumption, f_maize_own_consumption, cal_maize_quant_own_consumption_kg)

maize_price_subset <- Data %>% select(f_maize_price, cal_maize_measurement_prod, cal_maize_price)


#Sorghum
sorghum_quantities_produced_subset <- Data %>% select(f_sorghum_measurement_prod, cal_sorghum_measurement_prod, f_sorghum_quant_prod, cal_sorghum_quant_prod_kg)

sorghum_quantities_sold_subset <- Data %>% select(f_sorghum_measurement_sold, cal_sorghum_measurement_sold, f_sorghum_quant_sold, cal_sorghum_quant_sold_kg)

sorghum_quantities_lost_subset <- Data %>% select(f_sorghum_measurement_lost, cal_sorghum_lost_measurement, f_sorghum_quant_lost, cal_sorghum_quant_lost_kg)

sorghum_quantities_own_consumption_subset <- Data %>% select(f_sorghum_own_consumption_measurement, cal_sorghum_measurement_own_consumption, f_sorghum_own_consumption, cal_sorghum_quant_own_consumption_kg)

sorghum_price_subset <- Data %>% select(f_sorghum_price, cal_sorghum_measurement_prod, cal_sorghum_price)
```

```{r}
# Check how many seasons there are to adjust the following code 
table(Data$f_maize_rev_timeperiod)
table(Data$f_sorghum_rev_timeperiod)
```

```{r}
# harmonise season if not
# Data <- Data %>%
#   mutate(f_maize_rev_timeperiod=ifelse(f_maize_rev_timeperiod=="season 1", "season 1 march  july",
#                                        ifelse(f_maize_rev_timeperiod== "season 2", "season 2  august  february" ,
#                                               f_maize_rev_timeperiod)))%>%
#   mutate(f_sorghum_rev_timeperiod=ifelse(f_sorghum_rev_timeperiod=="season 1", "season 1 march  july",
#                                        ifelse(f_sorghum_rev_timeperiod== "season 2", "season 2  august  february" ,
#                                               f_sorghum_rev_timeperiod)))
```


```{r}

#  mutate(cal_focus_price = ifelse(cal_focus_quant_sold_kg > 0 & is.na(cal_focus_price), median_price_kgs$median_price_kgs, #cal_sorghum_price))

```

### 3.1) Maize crop revenue
...
Data <- Data %>%
  # Create new columns based on the season specified in f_maize_rev_timeperiod
  mutate(
    cal_maize_quant_prod_kg_season_1 = ifelse(f_maize_rev_timeperiod == "season 1 march  july", cal_maize_quant_prod_kg, NA),
    cal_maize_quant_prod_kg_season_2 = ifelse(f_maize_rev_timeperiod == "season 2  august  february", cal_maize_quant_prod_kg, NA),
    cal_maize_quant_prod_kg_whole_year = ifelse(f_maize_rev_timeperiod == "past 12 months whole year", cal_maize_quant_prod_kg, NA), #   
    cal_maize_quant_sold_kg_season_1 = ifelse(f_maize_rev_timeperiod == "season 1 march  july", cal_maize_quant_sold_kg, NA),
    cal_maize_quant_sold_kg_season_2 = ifelse(f_maize_rev_timeperiod == "season 2  august  february", cal_maize_quant_sold_kg, NA),
    cal_maize_quant_sold_kg_whole_year = ifelse(f_maize_rev_timeperiod == "past 12 months whole year", cal_maize_quant_sold_kg, NA),
#
    cal_maize_quant_own_consumption_kg_season_1 = ifelse(f_maize_rev_timeperiod == "season 1 march  july", cal_maize_measurement_own_consumption, NA),
    cal_maize_quant_own_consumption_kg_season_2 = ifelse(f_maize_rev_timeperiod == "season 2  august  february", cal_maize_measurement_own_consumption, NA),
    cal_maize_quant_own_consumption_kg_whole_year = ifelse(f_maize_rev_timeperiod == "past 12 months whole year", cal_maize_measurement_own_consumption, NA),
#
    cal_maize_quant_lost_kg_season_1 = ifelse(f_maize_rev_timeperiod == "season 1 march  july", cal_maize_quant_lost_kg, NA),
    cal_maize_quant_lost_kg_season_2 = ifelse(f_maize_rev_timeperiod == "season 2  august  february", cal_maize_quant_lost_kg, NA),
    cal_maize_quant_lost_kg_whole_year = ifelse(f_maize_rev_timeperiod == "past 12 months whole year", cal_maize_quant_lost_kg, NA)
  )
...
Data <- Data %>%
  ...
  group_by(identifier) %>%
  mutate(
    cal_maize_quant_prod_kg_season_1 = sum(na.omit(cal_maize_quant_prod_kg_season_1), na.rm = TRUE),
    cal_maize_quant_prod_kg_season_2 = sum(na.omit(cal_maize_quant_prod_kg_season_2), na.rm = TRUE),
    cal_maize_quant_prod_kg_whole_year = sum(na.omit(cal_maize_quant_prod_kg_whole_year), na.rm = TRUE),
    cal_maize_quant_sold_kg_season_1 = sum(na.omit(cal_maize_quant_sold_kg_season_1), na.rm = TRUE),
    cal_maize_quant_sold_kg_season_2 = sum(na.omit(cal_maize_quant_sold_kg_season_2), na.rm = TRUE),
    cal_maize_quant_sold_kg_whole_year = sum(na.omit(cal_maize_quant_sold_kg_whole_year), na.rm = TRUE), 
    cal_maize_quant_own_consumption_kg_season_1 = sum(na.omit(cal_maize_quant_own_consumption_kg_season_1), na.rm = TRUE),
    cal_maize_quant_own_consumption_kg_season_2 = sum(na.omit(cal_maize_quant_own_consumption_kg_season_2), na.rm = TRUE),
    cal_maize_quant_own_consumption_kg_whole_year = sum(na.omit(cal_maize_quant_own_consumption_kg_whole_year), na.rm = TRUE),
    cal_maize_quant_lost_kg_season_1 = sum(na.omit(cal_maize_quant_lost_kg_season_1), na.rm = TRUE),
    cal_maize_quant_lost_kg_season_2 = sum(na.omit(cal_maize_quant_lost_kg_season_2), na.rm = TRUE),
    cal_maize_quant_lost_kg_whole_year = sum(na.omit(cal_maize_quant_lost_kg_whole_year), na.rm = TRUE)
  ) %>% 
...
##Replace missing values in Maize revenue by zero
Data <- Data %>%
...
### 3.2) Sorghum crop revenue

```{r, include = FALSE}
Data <- Data %>%
  mutate(cal_sorghum_revenue = cal_sorghum_quant_sold_kg * cal_sorghum_price)

# Create extra columns to define the quantities per season
Data <- Data %>%
  # Create new columns based on the season specified in f_sorghum_rev_timeperiod
  mutate(
    cal_sorghum_quant_prod_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1", cal_sorghum_quant_prod_kg, NA),
    cal_sorghum_quant_prod_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2", cal_sorghum_quant_prod_kg, NA),
    cal_sorghum_quant_prod_kg_whole_year = ifelse(f_sorghum_rev_timeperiod == "past 12 months whole year", cal_sorghum_quant_prod_kg, NA), #   
    cal_sorghum_quant_sold_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1", cal_sorghum_quant_sold_kg, NA),
    cal_sorghum_quant_sold_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2", cal_sorghum_quant_sold_kg, NA),
    cal_sorghum_quant_sold_kg_whole_year = ifelse(f_sorghum_rev_timeperiod == "past 12 months whole year", cal_sorghum_quant_sold_kg, NA),
#
    cal_sorghum_quant_own_consumption_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1", cal_sorghum_measurement_own_consumption, NA),
    cal_sorghum_quant_own_consumption_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2", cal_sorghum_measurement_own_consumption, NA),
    cal_sorghum_quant_own_consumption_kg_whole_year = ifelse(f_sorghum_rev_timeperiod == "past 12 months whole year", cal_sorghum_measurement_own_consumption, NA),
#
    cal_sorghum_quant_lost_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1", cal_sorghum_quant_lost_kg, NA),
    cal_sorghum_quant_lost_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2", cal_sorghum_quant_lost_kg, NA),
    cal_sorghum_quant_lost_kg_whole_year = ifelse(f_sorghum_rev_timeperiod == "past 12 months whole year", cal_sorghum_quant_lost_kg, NA)
  )
...
Data <- Data %>%
  #Identify duplicates
  group_by(identifier, f_sorghum_rev_timeperiod) %>% 
  mutate(count_dup = n()) %>% # Count duplicates within each group
  mutate(count_dup = ifelse(is.na(f_sorghum_rev_timeperiod), NA, count_dup)) %>% # Check whether we don't make mistakes here. For some cases, farmers enter twice wrongly, while in other cases entering the same values for 2 seasons is ok.
  ...
  group_by(identifier) %>%
  mutate(
    cal_sorghum_quant_prod_kg_season_1 = sum(na.omit(cal_sorghum_quant_prod_kg_season_1), na.rm = TRUE),
    cal_sorghum_quant_prod_kg_season_2 = sum(na.omit(cal_sorghum_quant_prod_kg_season_2), na.rm = TRUE),
    cal_sorghum_quant_prod_kg_whole_year = sum(na.omit(cal_sorghum_quant_prod_kg_whole_year), na.rm = TRUE),
    cal_sorghum_quant_sold_kg_season_1 = sum(na.omit(cal_sorghum_quant_sold_kg_season_1), na.rm = TRUE),
    cal_sorghum_quant_sold_kg_season_2 = sum(na.omit(cal_sorghum_quant_sold_kg_season_2), na.rm = TRUE),
    cal_sorghum_quant_sold_kg_whole_year = sum(na.omit(cal_sorghum_quant_sold_kg_whole_year), na.rm = TRUE), 
    cal_sorghum_quant_own_consumption_kg_season_1 = sum(na.omit(cal_sorghum_quant_own_consumption_kg_season_1), na.rm = TRUE),
    cal_sorghum_quant_own_consumption_kg_season_2 = sum(na.omit(cal_sorghum_quant_own_consumption_kg_season_2), na.rm = TRUE),
    cal_sorghum_quant_own_consumption_kg_whole_year = sum(na.omit(cal_sorghum_quant_own_consumption_kg_whole_year), na.rm = TRUE),
    cal_sorghum_quant_lost_kg_season_1 = sum(na.omit(cal_sorghum_quant_lost_kg_season_1), na.rm = TRUE),
    cal_sorghum_quant_lost_kg_season_2 = sum(na.omit(cal_sorghum_quant_lost_kg_season_2), na.rm = TRUE),
    cal_sorghum_quant_lost_kg_whole_year = sum(na.omit(cal_sorghum_quant_lost_kg_whole_year), na.rm = TRUE)
  ) %>% 
  ...
  mutate(cal_sorghum_quant_own_consumption_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sorghum_quant_own_consumption_kg )) %>%
  ...
  mutate(cal_sorghum_quant_own_consumption_kg =  sum(cal_sorghum_quant_own_consumption_kg)) %>%
  ...
  mutate(cal_sorghum_quant_own_consumption_kg = ifelse(`repeat no` >1  , NA, cal_sorghum_quant_own_consumption_kg )) %>%
...
##Replace missing values in sorghum revenue by zero
Data <- Data %>%
  mutate(cal_sorghum_revenue = ifelse(is.na(cal_sorghum_revenue), 0, cal_sorghum_revenue)) 
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(cal_focus_revenue=cal_maize_revenue + cal_sorghum_revenue)
#subset
revenue_subset <- Data %>% select(cal_maize_revenue, cal_sorghum_revenue, cal_focus_revenue)
...
Data <- Data %>%
  ...
  mutate(cal_sorghum_productivity_acre = cal_sorghum_quant_prod_kg/f_sorghum_crop_size_acre)
```

### 5) Labour costs

For this case we need to refer to old calculations
...
# Run this code when a combination of labour activity and timeperiod is not selected
Data <- Data %>%
  mutate(f_labour_nurserymaint_paymentperactivity = 0,
...
@Not_asked_for_this_case then we used an old approach above
```{r}
...
```{r}
input_costs_subset <- Data %>% select(c(f_inputs_costs_seeds, f_inputs_costs_compost, f_inputs_costs_fertilizer, f_inputs_costs_chemicals, f_inputs_costs_beneficialinsects, f_inputs_costs_pesttraps, f_inputs_costs_irrigation, f_inputs_costs_other))
...
```{r}
Data <- Data %>%
  mutate(cal_focus_income = cal_maize_revenue - cal_focus_cost)
```
...
#    cal_actual_income = ifelse(is.na(cal_focus_income) | cal_focus_income == 0 | f_maize_quant_sold==0 | is.na(f_maize_quant_sold) #| f_maize_measurement_sold=="" , NA, cal_actual_income))
```
...
#    cal_actual_income_v2 = ifelse(is.na(cal_focus_income) | cal_focus_income == 0 | f_maize_quant_sold==0 | #is.na(f_maize_quant_sold) | f_maize_measurement_sold==""  | f_focus_income_share == 0, NA, cal_actual_income_v2))
...
subset <- Data %>% 
  select(cal_maize_quant_prod_kg, cal_sorghum_quant_prod_kg, cal_maize_quant_sold_kg, cal_sorghum_quant_sold_kg, cal_maize_price, cal_sorghum_price,
         cal_maize_quant_own_consumption_kg, cal_sorghum_quant_own_consumption_kg, cal_maize_quant_lost_kg, cal_sorghum_quant_lost_kg,
         cal_maize_revenue, cal_sorghum_revenue, cal_maize_productivity_acre, cal_sorghum_productivity_acre, cal_labour_cost, cal_inputs_costs, cal_focus_other_costs, cal_focus_cost, cal_focus_income, cal_othermaincrop_1_cost, cal_othermaincrop_2_cost, #cal_othermaincrop_production_value,
...
```{r, include = FALSE}
Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_level_2) %>%  ### the variable on the right differs from case to case!!!
          select(
...
NB:If the pi_ file is not available for basline, use the raw data and generate it. here is and example
```{r}
# Data_baseline<-read_excel("DA-627930161_2407 Landmark Millers (baseline).xlsx")
...
# 
# write.xlsx(pi_info, file="Pi_landmark_baseline.xlsx")
```

First load in the baseline farmers and only keep the phone numbers. 

```{r}
baseline_farmers <- read_excel("pi_2022_lmm_maize_sorghum.xlsx")
...
productivity <- Data %>%
    ...
    mutate(sorghum_product_min_sold = cal_sorghum_quant_prod_kg - cal_sorghum_quant_sold_kg) %>%
    ...
    select(identifier,
           ...
           f_sorghum_own_consumption,
           f_sorghum_own_consumption_measurement,
           f_sorghum_quant_lost,
           f_sorghum_measurement_lost,
           )
