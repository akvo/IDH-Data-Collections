...
```{r}
data_filename <- "DATA_ANALYSIS-SLogistics-beans.xlsx"
survey_filename <- "SURVEY_FORM-SLogistics-beans.xlsx"
```
...
#outlier focus crop size =75 replaced by NA
Data <- Data %>%
  mutate(f_focus_crop_size = ifelse(identifier == "45rh31twwva5", NA, f_focus_crop_size))
```
...
identifiers_to_replace <- c("npyyqm94qjar", "6059kk6g1qcb", "a4vwmpj7rw92", "cqgyy7vybv3c", "bxm6r220frd2", "7t6ds28q3ew7", "r4nwcm1wbvfw")
#now replace

Data <- Data %>%
  mutate(f_focus_price = ifelse(
    identifier %in% identifiers_to_replace & !is.na(median_price_kgs$median_price_kgs),
    median_price_kgs$median_price_kgs,
    f_focus_price
  ))
...
Data <- Data %>%
  mutate(
    f_focus_measurement_prod = case_when(
      identifier %in% c("unwb19wweqaf", "5e6ryhhfd332", "nuw8c3k343kv", "jd66gqub874b") ~ "90 kg bags",
      identifier %in% c("8tnya21xyhus", "e23rf18ne1cc", "d7fwpw7rb0ma", "nw24ga8k71rr") ~ NA_character_,
      TRUE ~ f_focus_measurement_prod
    ),
    f_focus_measurement_sold = case_when(
      identifier %in% c("jw0fp0vyb7q6", "8bmamhwmsckr", "uju7rhr7kbey") ~ "90 kg bags",
      identifier %in% c("8tnya21xyhus", "e23rf18ne1cc", "d7fwpw7rb0ma", "nw24ga8k71rr") ~ NA_character_,
      TRUE ~ f_focus_measurement_sold
    ),
    f_focus_own_consumption_measurement = case_when(
      identifier %in% c("gq2gsa6dr8v7", "x62cb52kd5k3") ~ "kgs",
...
Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier %in% c("m0sxtd8nnxmb", "d7fwpw7rb0ma", "9ct02s425xpd", "8bmamhwmsckr"), NA, f_focus_quant_prod),
         f_focus_quant_sold = ifelse(identifier %in% c("m0sxtd8nnxmb", "e23rf18ne1cc", "d7fwpw7rb0ma", "e23rf18ne1cc", "gp655v36w90y", "9ct02s425xpd", "8bmamhwmsckr"), NA,
                                      ifelse(identifier == "ya6ddwv7ngkh" & f_focus_quant_sold==6700, 1,
                                      ifelse(identifier == "uju7rhr7kbey" & f_focus_quant_sold==90, 1,
                                      ifelse(identifier == "gp655v36w90y", 1.75,
                                      ifelse(identifier == "9ct02s425xpd", 2.5, f_focus_quant_sold))))))
...
cal_otherinc_subset <- Data %>% select(c(cal_othermaincrop_1_inc_sold, cal_othermaincrop_1_cost, cal_othermaincrop_2_inc_sold, cal_othermaincrop_2_cost, cal_other_crop_income, cal_other_crop_income))
```
