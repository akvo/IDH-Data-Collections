...
## Rename your files
pi_filename <- "pi_farmworks.xlsx"
data_filename <- "353840002_farmworks_raw_data.xlsx"
survey_filename <- "Survey_farmworks_2022.xlsx"
data_delivery <- "2022_Farmworks_anom_french_beans_sweet_corn.xlsx"
...
case = "5215 Farmworks"
sdm_crop_1 = "French beans"
sdm_crop_2 = "Sweet corn"
country = "Kenya"
...
Data <- Data %>%
  ...
  #General case
  mutate(f_sweetcorn_harvest_num = as.numeric(f_harvest_num)) %>%
  mutate(f_frenchbeans_harvest_num = as.numeric(f_harvest_num_1)) %>%
...
Data <- Data %>% 
  ...
  mutate(f_size_acre = ifelse(f_unit_land == "squared meters", f_size*0.000247105, f_size),
         f_size_acre = ifelse(f_unit_land == "plot", f_size*0.125, f_size_acre),
         f_size_acre = ifelse(f_unit_land == "unit unit", f_size*0.125, f_size_acre),
         f_size_acre = ifelse(f_unit_land == "units units", f_size*0.125, f_size_acre)) %>%
  
  mutate(f_focus_crop_size_acre = ifelse(f_unit_land == "squared meters", f_focus_crop_size*0.000247105, f_focus_crop_size),
         f_focus_crop_size_acre = ifelse(f_unit_land == "plot", f_focus_crop_size*0.125, f_focus_crop_size_acre),
         f_focus_crop_size_acre = ifelse(f_unit_land == "unit unit", f_focus_crop_size*0.125, f_focus_crop_size_acre),
         f_focus_crop_size_acre = ifelse(f_unit_land == "units units", f_focus_crop_size*0.125, f_focus_crop_size_acre)) %>%
  
  mutate(f_size_othermaincrop_1_acre = ifelse(f_unit_land == "squared meters", 
                                              f_size_othermaincrop_1*0.000247105, 
                                              f_size_othermaincrop_1),
         f_size_othermaincrop_1_acre = ifelse(f_unit_land == "plot", 
                                              f_size_othermaincrop_1*0.125, 
                                              f_size_othermaincrop_1_acre),
         f_size_othermaincrop_1_acre = ifelse(f_unit_land == "unit unit", 
                                              f_size_othermaincrop_1*0.125, 
                                              f_size_othermaincrop_1_acre),
         f_size_othermaincrop_1_acre = ifelse(f_unit_land == "units units", 
                                              f_size_othermaincrop_1*0.125, 
                                              f_size_othermaincrop_1_acre)) %>%
  
  mutate(f_size_othermaincrop_2_acre = ifelse(f_unit_land == "squared meters", 
                                              f_size_othermaincrop_2*0.000247105, 
                                              f_size_othermaincrop_2),
         f_size_othermaincrop_2_acre = ifelse(f_unit_land == "plot", 
                                              f_size_othermaincrop_2*0.125, 
                                              f_size_othermaincrop_2_acre),
         f_size_othermaincrop_2_acre = ifelse(f_unit_land == "unit unit", 
                                              f_size_othermaincrop_2*0.125, 
                                              f_size_othermaincrop_2_acre),
         f_size_othermaincrop_2_acre = ifelse(f_unit_land == "units units", 
                                              f_size_othermaincrop_2*0.125, 
                                              f_size_othermaincrop_2_acre)) %>%
...
Data <- Data %>% 
  ...
  # Correct for farmers that gave total revenue instead of quantity sold
  mutate(f_focus_quant_sold = ifelse(f_focus_quant_sold == 17500, 700, f_focus_quant_sold),
         f_focus_quant_sold = ifelse(f_focus_quant_sold == 4250, 170, f_focus_quant_sold),
         f_focus_quant_sold = ifelse(f_focus_quant_sold == 1250, 50, f_focus_quant_sold),
         f_focus_quant_sold = ifelse(f_focus_quant_sold == 925, 37, f_focus_quant_sold)) %>%
  
  # Remove one farmer with odd numbers for both produced and sold 
  filter(identifier != "jgcpr5nbjyf2") %>%
  
  # Unrealistically high production numbers compared to farm size put to NA
  mutate(f_focus_quant_prod = ifelse(f_focus_quant_prod == 2000 & identifier != "eemwp1ma77xj", NA,f_focus_quant_prod),
         f_focus_quant_prod_1 = ifelse(f_focus_quant_prod_1 == 2200, NA, f_focus_quant_prod_1),
         f_focus_quant_prod_1 = ifelse(f_focus_quant_prod_1 == 5000, NA, f_focus_quant_prod_1)) %>%
  # Focus crop farm size = 0 --> impute with NA 
  mutate(f_focus_crop_size = ifelse(f_focus_crop_size == 0, NA, f_focus_crop_size),
         f_focus_crop_size_acre = ifelse(f_focus_crop_size_acre ==0, NA, f_focus_crop_size_acre)) 

Data <- Data %>%
  # Correct mistake: total revenue instead of focus crop price

mutate(        
        # Sweet corn
         f_focus_price = ifelse(f_focus_price > 1999, f_focus_price/f_focus_quant_sold, f_focus_price),
         # french beans
         f_focus_price_1 = ifelse(f_focus_price_1 > 3999, f_focus_price_1/f_focus_quant_sold_1, f_focus_price_1)) %>%
...
# NUMBER OF SEASONS
number_of_seasons_sweetcorn <- max(Data$f_sweetcorn_harvest_num, na.rm=TRUE)
number_of_seasons_frenchbeans <- max(Data$f_frenchbeans_harvest_num, na.rm=TRUE)
...
Data <- Data %>%
  ...
  mutate(cal_sweetcorn_measurement_prod := ifelse(
  ...
  mutate(cal_sweetcorn_measurement_sold := ifelse(
  ...
  mutate(cal_sweetcorn_lost_measurement := ifelse(
  ...
  mutate(cal_sweetcorn_measurement_own_consumption := ifelse(
  ...
  # FRENCH BEANS
  ##Measurement unit for production
  mutate(cal_frenchbeans_measurement_prod := ifelse(
    ...
    ##Measurement unit for sales
    mutate(cal_frenchbeans_measurement_sold := ifelse(
    ...
    # measurement unit for lost
    mutate(cal_frenchbeans_lost_measurement := ifelse(
    ...
    #measurement unit for own consumption
    mutate(cal_frenchbeans_measurement_own_consumption := ifelse(
...
# SWEETCORN
Data <- Data %>%
  mutate(cal_sweetcorn_quant_prod_kg = f_focus_quant_prod * cal_sweetcorn_measurement_prod) %>%
  mutate(cal_sweetcorn_quant_sold_kg = f_focus_quant_sold * cal_sweetcorn_measurement_sold) %>%
  mutate(cal_sweetcorn_quant_lost_kg = f_focus_quant_lost * cal_sweetcorn_lost_measurement) %>%
  mutate(cal_sweetcorn_quant_own_consumption_kg = f_focus_own_consumption * cal_sweetcorn_measurement_own_consumption) %>%
  
  mutate(cal_sweetcorn_price= f_focus_price)  
# FRENCH BEANS
Data <- Data %>%
  mutate(cal_frenchbeans_quant_prod_kg = f_focus_quant_prod_1 * cal_frenchbeans_measurement_prod) %>%
  mutate(cal_frenchbeans_quant_sold_kg = f_focus_quant_sold_1 * cal_frenchbeans_measurement_sold) %>%
  mutate(cal_frenchbeans_quant_lost_kg = f_focus_quant_lost_1 * cal_frenchbeans_lost_measurement) %>%
  mutate(cal_frenchbeans_quant_own_consumption_kg = f_focus_own_consumption_1 * cal_frenchbeans_measurement_own_consumption) %>%
  
  mutate(cal_frenchbeans_price= f_focus_price_1)  

##  ---- 3) Calc: Focus crop income  ----  
##  ---- 3a) Calc: Focus crop revenue ----  
Data <- Data %>%
  mutate(cal_sweetcorn_revenue = cal_sweetcorn_quant_sold_kg * cal_sweetcorn_price,
         cal_frenchbeans_revenue = cal_frenchbeans_quant_sold_kg * cal_frenchbeans_price)  
...
Data <- Data %>%
  ...
  mutate(cal_sweetcorn_quant_prod_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sweetcorn_quant_prod_kg)) %>%
  mutate(cal_sweetcorn_quant_sold_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sweetcorn_quant_sold_kg)) %>%
  mutate(cal_sweetcorn_quant_own_consumption_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sweetcorn_quant_own_consumption_kg )) %>%
  mutate(cal_sweetcorn_quant_lost_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sweetcorn_quant_lost_kg)) %>%
  mutate(cal_sweetcorn_revenue = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sweetcorn_revenue)) %>%
  
  #Sum the quantities for those that entered data for multiple seasons
  group_by(identifier, count_seasons) %>%
  mutate(cal_sweetcorn_quant_prod_kg =  sum(cal_sweetcorn_quant_prod_kg)) %>%
  mutate(cal_sweetcorn_quant_sold_kg =  sum(cal_sweetcorn_quant_sold_kg)) %>%
  mutate(cal_sweetcorn_quant_own_consumption_kg =  sum(cal_sweetcorn_quant_own_consumption_kg)) %>%
  mutate(cal_sweetcorn_quant_lost_kg =  sum(cal_sweetcorn_quant_lost_kg)) %>%
  mutate(cal_sweetcorn_revenue = sum(cal_sweetcorn_revenue)) %>%
  
  #Ensure we have 1 row of data for each farmer
  mutate(cal_sweetcorn_quant_prod_kg = ifelse(`repeat no` >1 , NA, cal_sweetcorn_quant_prod_kg)) %>%
  mutate(cal_sweetcorn_quant_sold_kg = ifelse(`repeat no` >1  , NA, cal_sweetcorn_quant_sold_kg)) %>%
  mutate(cal_sweetcorn_quant_own_consumption_kg = ifelse(`repeat no` >1  , NA, cal_sweetcorn_quant_own_consumption_kg )) %>%
  mutate(cal_sweetcorn_quant_lost_kg = ifelse(`repeat no` >1 , NA, cal_sweetcorn_quant_lost_kg)) %>%
  mutate(cal_sweetcorn_revenue = ifelse (`repeat no`>1, NA, cal_sweetcorn_revenue)) %>%
  ungroup() %>%
...
Data <- Data %>%
  ...
  #Replace values with 0 for those that entered the data twice 
  mutate(`repeat no` = as.numeric(`repeat no`)) %>%
  mutate(cal_frenchbeans_quant_prod_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_frenchbeans_quant_prod_kg)) %>%
  mutate(cal_frenchbeans_quant_sold_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_frenchbeans_quant_sold_kg)) %>%
  mutate(cal_frenchbeans_quant_own_consumption_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_frenchbeans_quant_own_consumption_kg )) %>%
  mutate(cal_frenchbeans_quant_lost_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_frenchbeans_quant_lost_kg)) %>%
  mutate(cal_frenchbeans_revenue = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_frenchbeans_revenue)) %>%
  
  #Sum the quantities for those that entered data for multiple seasons
  group_by(identifier, count_seasons) %>%
  mutate(cal_frenchbeans_quant_prod_kg =  sum(cal_frenchbeans_quant_prod_kg)) %>%
  mutate(cal_frenchbeans_quant_sold_kg =  sum(cal_frenchbeans_quant_sold_kg)) %>%
  mutate(cal_frenchbeans_quant_own_consumption_kg =  sum(cal_frenchbeans_quant_own_consumption_kg)) %>%
  mutate(cal_frenchbeans_quant_lost_kg =  sum(cal_frenchbeans_quant_lost_kg)) %>%
  mutate(cal_frenchbeans_revenue = sum(cal_frenchbeans_revenue)) %>%
  
  #Ensure we have 1 row of data for each farmer
  mutate(cal_frenchbeans_quant_prod_kg = ifelse(`repeat no` >1 , NA, cal_frenchbeans_quant_prod_kg)) %>%
  mutate(cal_frenchbeans_quant_sold_kg = ifelse(`repeat no` >1  , NA, cal_frenchbeans_quant_sold_kg)) %>%
  mutate(cal_frenchbeans_quant_own_consumption_kg = ifelse(`repeat no` >1  , NA, cal_frenchbeans_quant_own_consumption_kg )) %>%
  mutate(cal_frenchbeans_quant_lost_kg = ifelse(`repeat no` >1 , NA, cal_frenchbeans_quant_lost_kg)) %>%
  mutate(cal_frenchbeans_revenue = ifelse (`repeat no`>1, NA, cal_frenchbeans_revenue)) %>%
  ungroup() %>%
  select(-count_dup, -count_seasons)

## #Put revenue at 0 for those that do not have revenue or for non applicable rows
Data <- Data %>%
  mutate(cal_sweetcorn_revenue = ifelse(is.na(cal_sweetcorn_revenue), 0, cal_sweetcorn_revenue))  
Data <- Data %>%
  mutate(cal_frenchbeans_revenue = ifelse(is.na(cal_frenchbeans_revenue), 0, cal_frenchbeans_revenue))  

##  ---- 3c) Calc: Productivity  ----  
##Calculate productivity
Data <- Data %>%
  mutate(cal_sweetcorn_productivity_acre = cal_sweetcorn_quant_prod_kg/f_focus_crop_size_acre) %>%
  mutate(cal_frenchbeans_productivity_acre = cal_frenchbeans_quant_prod_kg/f_focus_crop_size_acre)
...
Data <- Data %>%
  ...
  mutate(cal_focus_income = (cal_sweetcorn_revenue + cal_frenchbeans_revenue) - cal_focus_cost)
...
unmatched <- list() # List is 0, so check!
...
#Is it a calculated variable? Than keep 
data$compare <-data$variable  %in%  vars_transformed$variable 
data <-  data %>%
  filter(compare == FALSE)    %>%
  select(-compare)

# Is it a variable from the question library? 
...
productivity <- Data %>%
  mutate(sweetcorn_product_min_sold = cal_sweetcorn_quant_prod_kg - cal_sweetcorn_quant_sold_kg,
         frenchbeans_product_min_sold = cal_frenchbeans_quant_prod_kg - cal_frenchbeans_quant_sold_kg) %>%
select(identifier,
       submitter,
       ...
       focus_crop,
       f_focus_quant_prod,
       f_focus_quant_prod_1,
       f_focus_measurement_prod,
       f_focus_measurement_prod_1,
       f_focus_crop_size,
       f_focus_crop_size_acre,
       cal_focus_productivity_acre,
       f_focus_quant_sold,
       f_focus_quant_sold_1,
       sweetcorn_product_min_sold,
       frenchbeans_product_min_sold)

write.xlsx (productivity, file = "productivity_check_2.xlsx", overwrite = TRUE)
