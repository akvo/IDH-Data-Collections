...
setwd("/Users/ineslesire/Documents/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/5215 Farmworks")
##  ---- Describe datafiles ----
...
# Note that you should keep the variable "submitter" for the time being so we can trace back which enumerator has entered that data. 
...
# Open the file productivity_check xlsx. Check the results, also using the intake form. Check with the data collection lead why certain numbers are odd. 
  ...
  # Here you make any context specific adjustments to the data
  # 1.
  # Example, let's say some farmers reported total price for a bag when they needed to report the price for the measurement unit they reported (e.g. kg) 
  # If you say your measurement unit is kg, f_focus_price should present the price/kg. If your measurement unit is a bag of 100 kg, f_focus_price should be the price for 100kg.
  # Below we show code that we used for a case where farmers in the Kichwamba region made this mistake.
  # Data <- Data %>% 
  # mutate(f_focus_price = ifelse(pi_location_cascade_sub_county == "kichwamba", f_focus_price/100, f_focus_price))
  # 2. 
  # Any variable name that has been changed, should also be changed in the codebook.
  # Example: 
# First rename the variable in the data, After that rename the variable in the survey_questions file. 
# Data <- Data %>%
...
Data <- Data %>%
  ...
  # Go for 2023, since we are approaching the end of the year so this will give more accurate results. 
  mutate(cal_hh_farmer_age = 2023 - hh_farmer_birthyear,
         cal_hh_head_age = 2023-hh_head_birthyear,
         cal_hh_member_birthyear = 2023-hh_member_birthyear) #update to current year, check whether applicable, not always included.
...
Data <- Data %>%
  mutate(cal_focus_productivity_acre = (ifelse(is.na(cal_sweetcorn_quant_prod_kg), 0, cal_sweetcorn_quant_prod_kg) + 
                                          ifelse(is.na(cal_frenchbeans_quant_prod_kg), 0, cal_frenchbeans_quant_prod_kg))
