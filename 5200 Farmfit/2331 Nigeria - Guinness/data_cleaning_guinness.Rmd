...
Data <- Data %>%
  ...
  mutate(f_focus_measurement_prod = ifelse(identifier == "v4ex6t3rmf5m", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "ec81xb82kd09", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "a42mn8x7mqm8", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "1a9uaux8btp1", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "1rgwm7bduvdm", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "tq17w1w2jhbf", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "5taahphh4agp", "85 kg bags", f_focus_measurement_prod)) %>%
  
  # Empty measurement unit. fill in same as sold/prod
  mutate(f_focus_measurement_prod = ifelse(identifier == "8yenxm6ruq3y", "kg", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_sold = ifelse(identifier == "3cseh1qf61tj", "150kg bag", f_focus_measurement_sold)) %>%
  
  # 2 farmers indicate ton as measurement unit, but this should be kg
  mutate(f_focus_measurement_prod = ifelse(identifier == "rd4q9darepff", "kg", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_prod = ifelse(identifier == "pqdk5nr1fhpa", "kg", f_focus_measurement_prod)) %>%
  
  # for some rows, the price is not the unit price --> multiply by 85 to get price per bag 
  mutate(f_focus_price = ifelse(identifier == "0e9cg4qhqqq3", f_focus_price*85, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "ggs73mx4hmpd", f_focus_price*85, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "qybdjtwvt6s9", f_focus_price*85, f_focus_price)) %>%

  # we derive from quantities and price variables that measurement unit should be 85kg bags instead of kg
  mutate(f_focus_measurement_prod = ifelse(identifier == "69qbyr1v1nue", "85 kg bags", f_focus_measurement_prod)) %>%
  mutate(f_focus_measurement_sold = ifelse(identifier == "69qbyr1v1nue", "85 kg bags", f_focus_measurement_sold)) %>%
  
  # Typos
  mutate(f_focus_price = ifelse(identifier == "tty8k3ad8r2p", 200, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "g68ax572sdh0", 190, f_focus_price)) %>%
  mutate(f_focus_price = ifelse(identifier == "wtxj4sd551dt", 190, f_focus_price)) %>%  
  mutate(f_focus_price = ifelse(identifier == "fy808g6jkx1c", 180, f_focus_price)) %>%  
  mutate(f_focus_price = ifelse(identifier == "4wu2hdyr55na", 200, f_focus_price)) %>%  
  
  mutate(f_focus_price = ifelse(identifier == "wwy8r5u3sgg9", f_focus_price/12, f_focus_price)) %>%  
  mutate(f_focus_price = ifelse(identifier == "197w538kptb3", 29000, f_focus_price)) %>%  
  
  mutate(f_focus_price = ifelse(identifier == "ggs73mx4hmpd", f_focus_price * 85, f_focus_price)) %>%  
  mutate(f_focus_price = ifelse(identifier == "qybdjtwvt6s9", f_focus_price * 85, f_focus_price)) %>%  
  mutate(f_focus_price = ifelse(identifier == "0e9cg4qhqqq3", f_focus_price * 85, f_focus_price)) %>%  
  
  mutate(f_focus_price = ifelse(identifier == "cremhy3huy7n", f_focus_price * 85, f_focus_price)) %>%  
  
  # production set equal to sold 
  mutate(f_focus_quant_sold = ifelse(identifier == "0qv44bxn2jey", 20, f_focus_quant_sold)) %>%  
  mutate(f_focus_quant_sold = ifelse(identifier == "u1w2bg78e6aq", 170, f_focus_quant_sold)) 
...
Data <- Data %>%
    #net income
    mutate(cal_livestock_income = f_livestock_income_total)
...
```{r, include = FALSE}
Data <- Data %>%
    mutate(cal_off_farm_labour_income = f_income_offfarmlabour_wage)
```
