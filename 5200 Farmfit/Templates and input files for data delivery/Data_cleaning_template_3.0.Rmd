...
# Raw data 
Data_raw <- read_excel(data_filename, sheet = "GCP data collection for Brasil")
Data_raw <- Data_raw %>%
  select(-starts_with("pi_"), -name_of_farmer, -mobile_number_farmer)

Data_activities_raw <- read_excel(data_filename, sheet = "questions_activities")
Data_labour_raw <- read_excel(data_filename, sheet = "hh_labour")
...
Data_labour_temporary <- Data_activities %>%
  ...
  mutate(cal_labour_temporary_cost_activity = ifelse(is.na(cal_labour_temporary_cost_activity), 0, cal_labour_temporary_cost_activity)) %>%
...
Data_labour_household <- Data_activities %>%
  ...
  mutate(cal_labour_cost_household_activity_1 = ifelse(is.na(cal_labour_cost_household_activity_1), 0, cal_labour_cost_household_activity_1))%>%
  group_by(`_parent_index`) %>%
...
Data <- Data %>%
  ...
  mutate(cal_labour_cost_acre = ifelse(is.na(cal_labour_cost_acre), 0, cal_labour_cost_acre))
...
Data <- Data %>%
  ...
  # Replace NA values with 0
  mutate(cal_labour_cost_incl_hhlabour_acre = ifelse(is.na(cal_labour_cost_incl_hhlabour_acre), 0, cal_labour_cost_incl_hhlabour_acre))
```
...
# Section 3 Below code Not yet reviewed
