...
```{r}
data_filename <- "DATA_ANALYSIS-647230002-farmworks endline.xlsx"
survey_filename <- "SURVEY_FORM-647230002-farmworks endline.xlsx"
```
...
@Adjust farm size and unit before converting

```{r}
#farm size in pole is too low. maybe in acres
Data <- Data %>%
...
Data <- Data %>%
  mutate(f_focus_quant_prod = 
           ifelse(f_focus_quant_prod == 9998, NA, 
                  ifelse(identifier %in% c("05m7kdaacc72", "8kv7kkrv303v"), f_focus_quant_sold,
                  f_focus_quant_prod)),
    f_focus_quant_sold = 
           ifelse(f_focus_quant_sold == 9998, NA,
                  ifelse(identifier == "qy054t3nad7j", f_focus_quant_sold / 10, f_focus_quant_sold)))

#odd quantities checked with enumerators
identifier_to_replace <- c("s0vh6chqv9uh", "5rp7pekh1aah", "8cjmdn4r8xng", "p6dv5bgps1t1", "7cwkp8hpfuv1", "u8bhk323cf14", "y0hab9gcwpp7", "aeg3k4pe3gfb")

Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier %in% identifier_to_replace, f_focus_quant_prod / 10, 
                                            ifelse(identifier =="05m7kdaacc72", 2500,
                                                   ifelse(identifier =="8kv7kkrv303v", 100,
                                                          f_focus_quant_prod))),
         f_focus_quant_sold = ifelse(identifier %in% identifier_to_replace, f_focus_quant_sold / 10, 
                                            ifelse(identifier =="05m7kdaacc72", 2500,
                                                   ifelse(identifier =="8kv7kkrv303v", 100,
                                                          f_focus_quant_sold))),
         f_focus_own_consumption = ifelse(identifier == "s0vh6chqv9uh", f_focus_own_consumption / 10,
...
#calculate median
 ...
 Data <- Data %>%
  mutate(f_focus_price = ifelse(f_focus_price <50 | f_focus_price > 60, median_price$median_price, f_focus_price))
...
```{r}
Data <- Data %>%
  mutate(f_size = ifelse(identifier == "g1xha21n1ytr", 400,
                         ifelse(identifier == "bq5maftws5nw", 600,
                                ifelse(identifier == "4chawxgqyaw0", 400,
                                       f_size))),
         f_focus_crop_size = ifelse(identifier == "g1xha21n1ytr", 400,
                         ifelse(identifier == "bq5maftws5nw", 600,
                                ifelse(identifier == "4chawxgqyaw0", 400,
                                    f_focus_crop_size)))
         )
...
Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier == "hwa1urcx9su0", 3000,
                                    ifelse(identifier == "4fym73pjta2t", 4400,
                                           ifelse(identifier == "yvp6fc90qk69", 6000,
                                                  ifelse(identifier == "8qn9u13c26fx", 4000,
                                                         ifelse(identifier == "abt2ycp5tqec", 3600,
                                                                ifelse(identifier == "g38y45870rjk", 3000,
                                                                       ifelse(identifier == "9vacr85rbvem", 4500,
                                    f_focus_quant_prod))))))),
         f_focus_quant_sold = ifelse(identifier == "hwa1urcx9su0", 3000,
                                    ifelse(identifier == "4fym73pjta2t", 4400,
                                                  ifelse(identifier == "yvp6fc90qk69", 6000,
                                                         ifelse(identifier == "8qn9u13c26fx", 4000,
                                                                ifelse(identifier == "abt2ycp5tqec", 3600,
