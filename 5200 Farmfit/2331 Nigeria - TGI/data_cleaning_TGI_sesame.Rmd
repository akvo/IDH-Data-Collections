---
title: "data_cleaning_TGI_sesame"
author: "Ines Lesire"
date: "2023-01-03"
...
pi_filename <- "pi_2023_TGI_sesame.xlsx"
data_filename <- "493890434_TGI_sesame_raw_data.xlsx"
survey_filename <- "survey_TGI_sesame.xlsx"
data_delivery <- "2023_TGI_anom_sesame.xlsx"
...
Data <- Data %>%
  # missing value in f_focus_measurement_sold
  mutate(f_focus_measurement_sold = ifelse(identifier == "uc3br1xc582d", "75 kg bag", f_focus_measurement_sold)) %>%
  # missing value in f_focus_price
  mutate(f_focus_price = ifelse(identifier == "cmk6319wq50d", 75000, f_focus_price)) %>%
  # missing value farm size, replace by median values
  mutate(f_focus_crop_size = ifelse(identifier == "9u5pb4fa3ev0", 2, f_focus_crop_size)) %>%
  # missing value f_focus_measurement_prod , replace by 75 kgs bag
  mutate(f_focus_measurement_prod = ifelse(identifier == "dvqucb09tdqt", "75 kgs bag", f_focus_measurement_prod)) %>%
  # Missing value quantity sold --> set equal to quantity produced
  mutate(f_focus_quant_sold = ifelse(identifier == "rckkpaa33f5j", 1.25, f_focus_quant_sold)) %>%
  mutate(f_focus_quant_sold = ifelse(identifier == "7d235j5vve1w", 3, f_focus_quant_sold)) %>%
  
# Outlier in price --> set equal to median 
  mutate(f_focus_price = ifelse(identifier == "7d235j5vve1w", 75000, f_focus_price))
```
...
```{r, include = FALSE}
# Replace missing values in cal_focus_measurement_prod with "75 kgs bag"
Data <- Data %>%
  mutate(f_focus_measurement_prod = ifelse(is.na(f_focus_measurement_prod), "75 kgs bag", f_focus_measurement_prod))
...
##Measurement unit for production
  ...
  mutate(cal_focus_measurement_prod = ifelse(f_focus_measurement_prod == "75 kgs bag", 75, cal_focus_measurement_prod)) 
...
##Measurement unit for sales focus crop
  ...
  mutate(cal_focus_measurement_sold = ifelse(f_focus_measurement_sold == "75 kg bags", 
                                             75, cal_focus_measurement_sold)) 
...
# measurement unit for lost
  ...
  mutate(cal_focus_lost_measurement = ifelse(f_focus_measurement_lost == "75 kg bags", 75, cal_focus_lost_measurement))
```
...
# measurement unit for lost
  ...
  mutate(cal_focus_measurement_own_consumption = ifelse(f_focus_own_consumption_measurement == "75 kg bags", 75, cal_focus_measurement_own_consumption))
```
...
Data <- Data %>%
  mutate(cal_equipment_costs = Data %>% 
           select(f_nonmech_equip_costs,f_materials_other_costs) %>% 
...
```{r, include = FALSE}
Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_state) %>%  ### the variable on the right differs from case to case!!!
          select(
...
write.xlsx(pi_info, file="pi_2023_TGI_sesame.xlsx")
...
write.xlsx (productivity, file = "productivity_check_sesame_2.xlsx", overwrite = TRUE)
```
