...
```{r}
data_filename <- "DATA_ANALYSIS-684550119-avanti-diary-2024.xlsx"
survey_filename <- "SURVEY_FORM-684550119-avanti-diary-2024.xlsx"
```
...
Data_raw <- Data_raw %>%
  select(-starts_with("pi_"), -name_of_farmer, -mobile_number_farmer, -Submitter)
...
### Change unit land measure

```{r}

Data <-Data %>%
  mutate(f_unit_land=ifelse(f_unit_land=="squared meters", "acres", f_unit_land),
  
         #change f_size and f_focus_size 0 for farmer who has no land
         
         f_size=ifelse(f_unit_land == "no land",0,f_size),
         f_focus_crop_size= ifelse(f_unit_land=="no land",0, f_focus_crop_size)
...
Data <- Data%>%
  mutate(
    #hh_farmer_birthyear= ifelse(hh_farmer_birthyear>2002 & hh_farmer_birthyear <9998,2024- (hh_farmer_birthyear/100) , hh_farmer_birthyear ),
    #     hh_head_birthyear = ifelse(hh_head_birthyear>2002 & hh_head_birthyear <9998, 2024 - (hh_head_birthyear/100), hh_head_birthyear)
    hh_farmer_birthyear=ifelse( hh_farmer_birthyear==1900,NA, hh_farmer_birthyear),
    hh_head_birthyear=ifelse(hh_head_birthyear==1900, NA, hh_head_birthyear)
...
### Remove respondent that refused to participate in the survey

```{r}

Data <-Data %>%
  filter(identifier !="1bm906yhetgh")
...
#### Adjustments for farm and focus farm size

```{r}

Data <- Data%>%
  mutate(f_size=ifelse(f_size>1000, f_size/1000, f_size), 
         f_size_acre=ifelse(f_size_acre>1000, f_size_acre/1000, f_size_acre), 
        f_focus_crop_size=ifelse(f_focus_crop_size>1000, f_focus_crop_size/1000, f_focus_crop_size),
        f_focus_crop_size_acre=ifelse(f_focus_crop_size_acre>1000, f_focus_crop_size_acre/1000, f_focus_crop_size_acre)
         )

Data <- Data %>%
 mutate( f_focus_crop_size=ifelse(identifier=="caucuhybma88" | identifier=="6s96aab3qwka", f_size, f_focus_crop_size) )

# Some enumerators a mistake on reporting the farm_size.

Data <- Data %>%
  mutate(f_focus_crop_size = ifelse(f_focus_crop_size>10, f_size,f_focus_crop_size),
         f_focus_crop_size_acre = ifelse(f_focus_crop_size_acre>10, f_size,f_focus_crop_size_acre),
...
### MERGE
We received feedback from the field that we preferred to update directly in dataset

```{r}

#Feedbaclk on the dataset. There were inconsistancies in the variables f_focus_quant_prod_day_nr, f_focus_quant_prod_day, f_focus_quant_sold_per_cow, f_focus_quant_sold et f_focus_price. ///
# After, checking with the field coordinators we noticed that :
#1. The enumerator accidently pressed multiple zeroes
#2. In a few cases, the enumertors perhaps misunderstood the question instead of asking for unit price/litre they got the total price. 

data_feedback <- read_excel(("corrected_dataset.xlsx"), sheet="sheet1")


merged_df <- merge(Data, data_feedback, by = "identifier", all.x = TRUE, suffixes = c("", ".update"))

Data <- merged_df %>%
  mutate(f_focus_quant_prod_day_nr = coalesce(f_focus_quant_prod_day_nr.update, f_focus_quant_prod_day_nr),
         f_focus_quant_prod_day = coalesce(f_focus_quant_prod_day.update,f_focus_quant_prod_day),
         f_focus_quant_sold_per_cow= coalesce(f_focus_quant_sold_per_cow.update, f_focus_quant_sold_per_cow),
         f_focus_quant_sold = coalesce(f_focus_quant_sold.update,f_focus_quant_sold),
         f_focus_price = coalesce(f_focus_price.update, f_focus_price)
...
#         ) %>%
  select(-ends_with(".update"))


#updated_df <- merged_df %>%
 # mutate(
  #  value1 = coalesce(value1.update, value1),
   # value2 = coalesce(value2.update, value2),
    #value3 = coalesce(value3.update, value3)
...
##Measurement unit for production
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      ...
      ifelse(f_focus_measurement_prod %in%c("litres"), 1, 0)
      ))
...
**Measurement unit sold**

```{r, include = FALSE}

Data <- Data %>%
##Measurement unit for production
  mutate(cal_focus_measurement_sold := ifelse(grepl("[[:digit:]]", f_focus_measurement_sold), 
      extract_numeric(f_focus_measurement_sold), 
      ifelse(f_focus_measurement_sold %in%c("litres"), 1, 0)
      ))
...
# measurement unit for lost
  mutate(cal_focus_lost_measurement := ifelse(
    ...
    ifelse(f_focus_measurement_lost %in%c("litres"), 1, 0)))
...
# measurement unit for lost
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in%c("litres"), 1, 0))) 
```
...
Data <- Data %>%
  mutate(cal_focus_quant_prod_day =  f_focus_quant_prod_day * cal_focus_measurement_prod*f_focus_quant_prod_day_nr)%>%
  ...
  mutate(cal_focus_quant_own_consumption =  f_focus_own_consumption * cal_focus_measurement_own_consumption)%>%
  
  mutate_at(vars(starts_with("cal_focus_quant")), ~replace_na(., 0)) 
    
  # also calculate the unit price 
Data <- Data %>%
  mutate(cal_focus_price =  f_focus_price/cal_focus_measurement_sold)
```
...
quantities_produced_subset <- Data %>% select(f_focus_measurement_prod, cal_focus_measurement_prod, cal_focus_quant_prod_day)

quantities_sold_subset <- Data %>% select(f_focus_measurement_sold, cal_focus_measurement_sold, cal_focus_quant_sold)

quantities_lost_subset <- Data %>% select(f_focus_measurement_lost, cal_focus_lost_measurement, f_focus_quant_lost)

quantities_own_consumption_subset <- Data %>% select(f_focus_own_consumption_measurement, cal_focus_measurement_own_consumption, f_focus_own_consumption)

price_subset <- Data %>% select(f_focus_price, cal_focus_measurement_prod)
```
...
#harmonise season if not
 Data <- Data %>%
   mutate(f_focus_rev_timeperiod=ifelse(f_focus_rev_timeperiod=="season 1 dry season", "season 1",
                                        ifelse(f_focus_rev_timeperiod== "season 2 wet season", "season 2" ,
                                               f_focus_rev_timeperiod)))
```

```{r}
Data <- Data %>%
  mutate(cal_focus_price = ifelse(cal_focus_quant_sold > 0 & is.na(cal_focus_price), median(cal_focus_price), cal_focus_price))
```
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(f_focus_other_inc_monthly = ifelse(is.na(f_focus_other_inc_monthly), 0, f_focus_other_inc_monthly)) %>%
  mutate(f_focus_sold_num  = ifelse(is.na(f_focus_sold_num ), 0, f_focus_sold_num )) %>%
  mutate(f_focus_sold_price  = ifelse(is.na(f_focus_sold_price ), 0, f_focus_sold_price )) %>%
  
  mutate(cal_focus_revenue = cal_focus_quant_sold * cal_focus_price + (f_focus_other_inc_monthly * 12) + (f_focus_sold_num * f_focus_sold_price	) )
...
Data <- Data %>%
  ...
  mutate(
    cal_focus_quant_prod_day_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_prod_day, NA),
    cal_focus_quant_prod_day_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_prod_day, NA),

    #   
    cal_focus_quant_sold_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_sold, NA),
    cal_focus_quant_sold_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_sold, NA),

#
    cal_focus_quant_own_consumption_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_own_consumption, NA),
    cal_focus_quant_own_consumption_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_own_consumption, NA),

#
    cal_focus_quant_lost_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_lost, NA),
    cal_focus_quant_lost_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_lost, NA),
...
Data <- Data %>%
  ...
  group_by(identifier) %>%
  mutate(
    cal_focus_quant_prod_day_season_1 = sum(na.omit(cal_focus_quant_prod_day_season_1), na.rm = TRUE),
    cal_focus_quant_prod_day_season_2 = sum(na.omit(cal_focus_quant_prod_day_season_2), na.rm = TRUE),

    cal_focus_quant_sold_season_1 = sum(na.omit(cal_focus_quant_sold_season_1), na.rm = TRUE),
    cal_focus_quant_sold_season_2 = sum(na.omit(cal_focus_quant_sold_season_2), na.rm = TRUE),

    cal_focus_quant_own_consumption_season_1 = sum(na.omit(cal_focus_quant_own_consumption_season_1), na.rm = TRUE),
    cal_focus_quant_own_consumption_season_2 = sum(na.omit(cal_focus_quant_own_consumption_season_2), na.rm = TRUE),

    cal_focus_quant_lost_season_1 = sum(na.omit(cal_focus_quant_lost_season_1), na.rm = TRUE),
    cal_focus_quant_lost_season_2 = sum(na.omit(cal_focus_quant_lost_season_2), na.rm = TRUE),

  ) %>% 
  
  #Replace values with 0 for those that entered the data twice 
  mutate(`repeat no` = as.numeric(`repeat no`)) %>%
  mutate(cal_focus_quant_prod_day = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_prod_day)) %>%
  mutate(cal_focus_quant_sold = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_sold)) %>%
  mutate(cal_focus_quant_own_consumption = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_own_consumption )) %>%
  mutate(cal_focus_quant_lost = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_lost)) %>%
  ...
  mutate(cal_focus_quant_prod_day =  sum(cal_focus_quant_prod_day)) %>%
  ...
  mutate(cal_focus_quant_prod_day = ifelse(`repeat no` >1 , NA, cal_focus_quant_prod_day)) %>%
  mutate(cal_focus_quant_sold = ifelse(`repeat no` >1  , NA, cal_focus_quant_sold)) %>%
  mutate(cal_focus_quant_own_consumption = ifelse(`repeat no` >1  , NA, cal_focus_quant_own_consumption )) %>%
  mutate(cal_focus_quant_lost = ifelse(`repeat no` >1 , NA, cal_focus_quant_lost)) %>%
...
Data <- Data %>%
  mutate(cal_focus_productivity_day =  cal_focus_quant_prod_day / f_focus_quant_prod_day_nr)
```
...
#harmonise season if not
Data <- Data %>%
  mutate(f_focus_inputs_rev_timeperiod=ifelse(f_focus_inputs_rev_timeperiod=="season 1 dry season", "season 1",
                                       ifelse(f_focus_inputs_rev_timeperiod== "season 2 wet season", "season 2" ,
...
```{r}
input_costs_subset <- Data %>% select(c(f_inputs_costs_feed_season,f_inputs_costs_veterinary_season,f_inputs_costs_nutrition_season,f_inputs_costs_bedding_season,f_inputs_costs_wsupply_season,f_inputs_costs_cleaning_season, f_inputs_costs_milkequip_season, f_inputs_costs_breeding_season,f_inputs_costs_mainten_season,f_inputs_costs_energy_season ))
...
```{r}
# Fill all NA values 
variables_to_replace <- c("f_focus_costs_feed","f_focus_costs_veterinary","f_focus_costs_nutritional_suppl","f_focus_costs_bedding","f_focus_costs_watersupplymanagement","f_focus_costs_waterdrinking","f_focus_costs_milking","f_focus_costs_cleaning","f_focus_costs_breeding","f_focus_costs_maintenance_housing","f_focus_costs_energy","f_costs_land_focuscrop","f_focus_costs_storage","f_focus_costs_marketing","f_focus_costs_transportation")
...
# Calculation
Data <- Data %>%
  mutate(cal_focus_other_costs = f_focus_costs_feed+f_focus_costs_veterinary+f_focus_costs_nutritional_suppl+f_focus_costs_bedding+f_focus_costs_watersupplymanagement+f_focus_costs_waterdrinking+f_focus_costs_milking+f_focus_costs_cleaning+f_focus_costs_breeding+f_focus_costs_maintenance_housing+f_focus_costs_energy+f_costs_land_focuscrop+f_focus_costs_storage+f_focus_costs_marketing+f_focus_costs_transportation)
```
...
#  mutate_at(vars(all_of(variables_to_replace)), ~replace_na(., 0))%>%
  ...
  mutate (f_equip_costs = replace(as.numeric(f_equip_costs), is.na(f_equip_costs), 0) )
  #mutate(f_equip_costs=ifelse(is.na(f_equip_costs),0,f_equip_costs))
...
subset <- Data %>% 
  select(cal_focus_quant_prod_day, cal_focus_price,cal_focus_quant_own_consumption,cal_focus_quant_lost,
         cal_focus_revenue, cal_focus_productivity_day, cal_labour_cost, cal_inputs_costs, 
...
```{r, include = FALSE}
Data <- Data %>%
        #  rename(location_cascade_State = pi_location_cascade_State) %>%  ### the variable on the right differs from case to case!!!
          select(
...
#  Skip the variables with no data
   if (j == "f_crop_labour_types") {
     next
   }
   if (j == "f_focus_labour_hired_activities") {
     next
   }
...
```{r}
original_list_diary <- read_excel("stellapps_original_sample.xlsx", sheet = "Sheet1") 

# Merge the datasets and create the dummy variable
Data <- Data %>%
  left_join(original_list_diary, by = "mobile_number_farmer") %>% # Merge based on phone_number
  mutate(original_sample = if_else(!is.na(mobile_number_farmer), "yes","no" )) # Create the dummy variable
...
productivity <- Data %>%
    mutate(focus_product_min_sold = cal_focus_quant_prod_day - cal_focus_quant_sold) %>%
    ...
    select(identifier,
         submitter,
           ...
           f_focus_quant_prod_day_nr,
           cal_focus_productivity_day,
           f_focus_quant_prod_day,
           f_focus_measurement_prod,
           cal_focus_quant_prod_day,
           f_focus_quant_sold_per_cow ,
         f_focus_quant_sold_per_cow,
