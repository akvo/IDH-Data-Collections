...
```{r}
data_filename <- "DATA_ANALYSIS-684560003-avanti-carrots-2024.xlsx"
survey_filename <- "SURVEY_FORM-684560003-avanti-carrots-2024.xlsx"
```
...
Data <- Data%>%
  mutate(hh_farmer_birthyear= ifelse(hh_farmer_birthyear>2002 & hh_farmer_birthyear <9998,2024- (hh_farmer_birthyear/100) , hh_farmer_birthyear ),
         hh_head_birthyear = ifelse(hh_head_birthyear>2002 & hh_head_birthyear <9998, 2024 - (hh_head_birthyear/100), hh_head_birthyear),
         
# This farmer age is incorrect. We prefer to put it as missing
  hh_farmer_birthyear = ifelse(identifier=="tcb4fsm18kf5", 9998,hh_farmer_birthyear ),
  hh_head_birthyear = ifelse(identifier=="tcb4fsm18kf5", 9998,hh_head_birthyear)
...
data_feedback <- read_excel(("corrected_dataset.xlsx"), sheet="corrected")


merged_df <- merge(Data, data_feedback, by = "identifier", all.x = TRUE, suffixes = c("", ".update"))

Data <- merged_df %>%
  mutate(f_focus_quant_prod = coalesce(f_focus_quant_prod.update, f_focus_quant_prod),
         f_focus_measurement_prod = coalesce(f_focus_measurement_prod.update,f_focus_measurement_prod),
         f_focus_quant_sold = coalesce(f_focus_quant_sold.update, f_focus_quant_sold),
         f_focus_measurement_sold = coalesce(f_focus_measurement_sold.update,f_focus_measurement_sold),
         f_focus_price = coalesce(f_focus_price.update, f_focus_price),
         f_focus_measurement_lost = coalesce(f_focus_measurement_lost.update, f_focus_measurement_lost)
...
```{r}
Data <- Data %>%
  mutate(f_focus_quant_prod= ifelse(f_focus_quant_prod== 1 & f_focus_measurement_prod=="80kg bag" & f_focus_quant_prod != f_focus_quant_sold, f_focus_quant_sold, f_focus_quant_prod))
...
Data <- Data %>%
  mutate(
    #prod
    f_focus_measurement_prod = ifelse(f_focus_measurement_prod=="",f_focus_measurement_sold, f_focus_measurement_prod),

    #sold
   f_focus_measurement_sold = ifelse(f_focus_measurement_sold=="",f_focus_measurement_prod, f_focus_measurement_sold),
 # f_focus_measurement_sold = ifelse(f_focus_measurement_sold=="kg",f_focus_measurement_prod, f_focus_measurement_sold )
  )


```

###Quatity sold measurement unit

```{r}

identifier_replace <- c("63dsjwmbm2ae", "76u2pagynwwn", "aferuwc9k1ky", "bebj1g2fuy7s", "daaxfyat31ff" , "pbpn4ejvakvv" , "qj2qd1ep1912" , "sa4ukfb2heje" , "ufwvr6r0mtuw", "myngtkvvg7ay")

Data <- Data %>%
  mutate(f_focus_measurement_sold = ifelse(identifier %in% identifier_replace, "kg", f_focus_measurement_sold))


Data <- Data %>%
  mutate(f_focus_quant_sold= ifelse(identifier=="f_focus_quant_sold",15000, f_focus_quant_sold))
...
##Measurement unit for production
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      ...
      ifelse(f_focus_measurement_prod %in%c("kg box", "kg bag","kg"), 1, 0)))
```
...
##Measurement unit for sales focus crop
  mutate(cal_focus_measurement_sold := ifelse(
      ...
      ifelse(f_focus_measurement_sold %in%c("kg box", "kg bag","kg"), 1, 0)))
...
# measurement unit for lost
  mutate(cal_focus_lost_measurement := ifelse(
    ...
    ifelse(f_focus_measurement_lost %in%c("kg box", "kg bag","kg"), 1, 0)))
```
...
# measurement unit for lost
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in%c("kg box", "kg bag", "kg"), 1, 0))) 
```
...
Data <- Data %>%
  mutate(cal_focus_price_kg = ifelse(cal_focus_measurement_prod != 0 , f_focus_price, 0))%>%

         
   mutate(cal_focus_price_kg = ifelse(is.na(cal_focus_quant_sold_kg) | cal_focus_quant_sold_kg == 0, NA, cal_focus_price_kg))%>%
...
price_subset <- Data %>% select(f_focus_price, cal_focus_measurement_prod, cal_focus_price_kg)
```
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_prod_kg_season_3 = ifelse(f_focus_rev_timeperiod == "season 3", cal_focus_quant_prod_kg, NA),
    ...
    cal_focus_quant_sold_kg_season_3 = ifelse(f_focus_rev_timeperiod == "season 3", cal_focus_quant_sold_kg, NA),
...
#
    ...
    cal_focus_quant_own_consumption_kg_season_3 = ifelse(f_focus_rev_timeperiod == "season 3", cal_focus_quant_own_consumption_kg, NA),
...
#
    ...
    cal_focus_quant_lost_kg_season_3 = ifelse(f_focus_rev_timeperiod == "season 3", cal_focus_quant_lost_kg, NA),
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_prod_kg_season_3 = sum(na.omit(cal_focus_quant_prod_kg_season_3), na.rm = TRUE),
    ...
    cal_focus_quant_sold_kg_season_3 = sum(na.omit(cal_focus_quant_sold_kg_season_3), na.rm = TRUE),
    ...
    cal_focus_quant_own_consumption_kg_season_3 = sum(na.omit(cal_focus_quant_own_consumption_kg_season_3), na.rm = TRUE),
    ...
    cal_focus_quant_lost_kg_season_3 = sum(na.omit(cal_focus_quant_lost_kg_season_3), na.rm = TRUE),
...
Data <- Data %>%
  mutate(cal_focus_productivity_kg_acre =  cal_focus_quant_prod_kg / f_focus_crop_size_acre)
```
...
subset <- Data %>% 
  select(cal_focus_quant_prod_kg,  cal_focus_quant_sold_kg, 
         cal_focus_price_kg, 
         cal_focus_quant_own_consumption_kg,  
         cal_focus_quant_lost_kg, 
         cal_focus_revenue, cal_focus_productivity_kg_acre, cal_labour_cost, cal_inputs_costs, 
...
#Merge the Data with the farmer original list in order dto create a variable that indicate if the farmers are from the original list of not



```{r}
original_list_carrots <- read_excel("leaf_original_sample.xlsx", sheet = "Sheet1") 

# Merge the datasets and create the dummy variable
Data <- Data %>%
  left_join(original_list_carrots, by = "mobile_number_farmer") %>% # Merge based on phone_number
