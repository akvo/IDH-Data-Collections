...
```{r}
case = "Afro-kai-Limited"
year = "2024"
sdm_crop = "maize_and_sorghum"
country = "Uganda"
```
...
```{r}
data_filename <- "DATA_ANALYSIS-640060095-AfroKai-maize-sorghum.xlsx"
survey_filename <- "SURVEY_FORM-640060095-AfroKai-maize-sorghum-merged.xlsx"
data_additional_name <- "DATA_ANALYSIS-Afrokai - Endline questions-Maize and Sorghum.xlsx"
...
#crop size > farm size
Data <- Data %>%
  mutate(f_size = ifelse(identifier %in% c("dj2g8k9c1rwg", "jjvv61wvpma8"), f_maize_crop_size + f_sorghum_crop_size, f_size))


Data <- Data %>%
  mutate(f_size = ifelse(identifier == "ejn6nmmtfxdh", 5, f_size),
         f_maize_crop_size= ifelse(identifier == "ax4c2ttve569", 5.5,
                                   ifelse(identifier == "866eb0vsyrs2", 7,
                                          ifelse(identifier == "g8b0v2rbx9f1", 5,
                                                 ifelse(identifier == "mtcwybb7rm4r", 4,
                                                        ifelse(identifier == "gc6j7m2vuh1w", 5,
                                                               ifelse(identifier == "2px78hfquda5", 5,
                                                                      ifelse(identifier == "q2vq99hu769v", 5,f_maize_crop_size))))))))
...
#        na.rm = TRUE, remove = FALSE, sep=" ") %>%
  ...
  mutate(f_size_acre = ifelse(f_unit_land == "hectares", f_size* 2.471, 
                              ifelse(f_unit_land == "squared meters",  f_size / 4047,
                                     ifelse(f_unit_land == "vara" , f_size / 643.51925488,
                              f_size)))) %>%
  ...
  mutate(f_maize_crop_size_acre = ifelse(f_unit_land == "hectares", f_maize_crop_size * 2.471, 
                                         ifelse(f_unit_land == "squared meters",  f_maize_crop_size / 4047,
                                                ifelse(f_unit_land == "vara" , f_maize_crop_size / 643.51925488,
                                         f_maize_crop_size)))) %>%
  mutate(f_sorghum_crop_size_acre = ifelse(f_unit_land == "hectares", f_sorghum_crop_size * 2.471, 
                                           ifelse(f_unit_land == "squared meters",  f_sorghum_crop_size / 4047,
                                                ifelse(f_unit_land == "vara" , f_sorghum_crop_size / 643.51925488,
                                           f_sorghum_crop_size)))) %>%
  ...
  mutate(f_size_hectare = ifelse(f_unit_land == "hectares", f_size, 
                                 ifelse(f_unit_land == "squared meters",  f_size / 10000,
                                      ifelse(f_unit_land == "vara" , f_size / 14233.21304608,
                                 f_size_acre / 2.471)))) %>%
  #mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "hectares", f_focus_crop_size, f_focus_crop_size_acre / 2.471))%>%
  mutate(f_maize_crop_size_hectare = ifelse(f_unit_land == "hectares", f_maize_crop_size, 
                                            ifelse(f_unit_land == "squared meters",  f_maize_crop_size / 10000,
                                                  ifelse(f_unit_land == "vara" , f_maize_crop_size / 14233.21304608,
                                            f_maize_crop_size_acre / 2.471)))) %>%
  mutate(f_sorghum_crop_size_hectare = ifelse(f_unit_land == "hectares", f_sorghum_crop_size, 
                                              ifelse(f_unit_land == "squared meters",  f_sorghum_crop_size / 10000,
                                                  ifelse(f_unit_land == "vara" , f_sorghum_crop_size / 14233.21304608,
                                              f_sorghum_crop_size_acre / 2.471))))
...
#### delete wrong obsevations
```{r}
Data <- subset(Data, identifier != "qmyrc5cfge8x")
```
...
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#Sorghum quantity in 50 kg bags but price is given per kgs. then convert quant to kgs. also harmonise others in kgs in order to protect from outlier function
unit_replace <- "50 kg bags"
Data <- Data %>%
  mutate(f_sorghum_quant_sold = ifelse(f_sorghum_measurement_sold == unit_replace, f_sorghum_quant_sold * 50, f_sorghum_quant_sold),
    f_sorghum_measurement_sold = ifelse(f_sorghum_measurement_sold == unit_replace, "kgs", f_sorghum_measurement_sold),
         f_sorghum_quant_prod = ifelse(f_sorghum_measurement_prod == unit_replace, f_sorghum_quant_prod * 50, f_sorghum_quant_prod),
         f_sorghum_measurement_prod = ifelse(f_sorghum_measurement_prod == unit_replace, "kgs", f_sorghum_measurement_prod)) %>%
#Do the same for consumption and lost
  mutate(f_sorghum_own_consumption = ifelse(f_sorghum_own_consumption_measurement == unit_replace, f_sorghum_own_consumption * 50, f_sorghum_own_consumption),
    f_sorghum_own_consumption_measurement = ifelse(f_sorghum_own_consumption_measurement == unit_replace, "kgs", f_sorghum_own_consumption_measurement),
    #losses are in 100 kg bags
         f_sorghum_quant_lost = ifelse(f_sorghum_measurement_lost == unit_replace, f_sorghum_quant_lost * 100, f_sorghum_quant_lost),
         f_sorghum_measurement_lost = ifelse(f_sorghum_measurement_lost == unit_replace, "kgs", f_sorghum_measurement_lost)
             )
...
Data <- Data %>%
  mutate(f_maize_quant_sold = ifelse(f_maize_measurement_sold == unit_replace, f_maize_quant_sold * 50, f_maize_quant_sold),
           f_maize_price = ifelse(f_maize_measurement_sold == unit_replace, f_maize_price / 50, f_maize_price),
    f_maize_measurement_sold = ifelse(f_maize_measurement_sold == unit_replace, "kgs", f_maize_measurement_sold),
         f_maize_quant_prod = ifelse(f_maize_measurement_prod == unit_replace, f_maize_quant_prod * 50, f_maize_quant_prod),
         f_maize_measurement_prod = ifelse(f_maize_measurement_prod == unit_replace, "kgs", f_maize_measurement_prod)) %>%
#Do the same for consumption and lost
  mutate(f_maize_own_consumption = ifelse(f_maize_own_consumption_measurement == unit_replace, f_maize_own_consumption * 50, f_maize_own_consumption),
    f_maize_own_consumption_measurement = ifelse(f_maize_own_consumption_measurement == unit_replace, "kgs", f_maize_own_consumption_measurement),
    #losses are in 100 kg bags
         f_maize_quant_lost = ifelse(f_maize_measurement_lost == unit_replace, f_maize_quant_lost * 100, f_maize_quant_lost),
         f_maize_measurement_lost = ifelse(f_maize_measurement_lost == unit_replace, "kgs", f_maize_measurement_lost)
             )
...
Data <- Data %>%
  mutate(f_sorghum_measurement_sold = ifelse(f_sorghum_quant_sold==0, NA_character_ , f_sorghum_measurement_sold),
         ...
         f_maize_measurement_sold = ifelse(f_maize_quant_sold==0, NA_character_ , f_maize_measurement_sold),
...
#maize
Data <- Data %>%
  #production lower than sold. checked regarding the land size
  mutate(f_maize_quant_prod = ifelse(identifier %in% c("wrxrmkqrd44a", "r551e8q23m9r", "uy0f7vcf7v6r", "h764dg1hb1t6"), f_maize_quant_sold, f_maize_quant_prod),
         f_maize_quant_sold = ifelse(identifier== "ap6x470qky2c", f_maize_quant_prod, f_maize_quant_sold))

#sorghum
Data <- Data %>%
  #production lower than sold. checked regarding the land size and do sold=prod
  mutate(f_sorghum_quant_sold = ifelse(identifier %in% c("yyjfex985stc", "73tgsmj1srre"), f_sorghum_quant_prod, f_sorghum_quant_sold))
```
...
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
# some adjustements are made when converting quantities and units. see code above fore maize
```
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(f_maize_quant_prod= ifelse(identifier == "3rbmh5cs994d", 46120, 
                                     ifelse(identifier == "wgpq37ajk9v1", 25000, 
                                            ifelse(identifier == "b6db053vnkbm", 10000, 
                                     f_maize_quant_prod))))

Data <- Data %>%
  mutate(f_maize_quant_sold= ifelse(identifier == "3rbmh5cs994d", 40000, 
                                     ifelse(identifier == "wgpq37ajk9v1", 25000, 
                                            ifelse(identifier == "cmj46nabpd76", 19700, 
                                     f_maize_quant_sold))))


Data <- Data %>%
  mutate(f_size= ifelse(identifier == "dj2g8k9c1rwg", 1.5, 
                                     ifelse(identifier == "b391u7fr07bt", 45, 
                                            ifelse(identifier == "rga3bd11j2h7", 60, 
                                                   ifelse(identifier == "3rbmh5cs994d", 54,
                                                          ifelse(identifier == "r1uk1n7duyvq", 40,
                                                                 ifelse(identifier == "7f66j1vrm58a", 80,
                                                                        ifelse(identifier == "rypqnw2wd4ua", 50,
                                                                               ifelse(identifier == "jjvv61wvpma8", 95,
                                     f_size)))))))),
                        f_maize_crop_size= ifelse(identifier == "dj2g8k9c1rwg", 1.5, 
                                     ifelse(identifier == "b391u7fr07bt", 1.5, 
                                            ifelse(identifier == "rga3bd11j2h7", 45, 
                                                   ifelse(identifier == "3rbmh5cs994d", 47,
                                                          ifelse(identifier == "r1uk1n7duyvq", 4,
                                                                 ifelse(identifier == "7f66j1vrm58a", 80,
                                                                        ifelse(identifier == "rypqnw2wd4ua", 50,
                                                                               ifelse(identifier == "jjvv61wvpma8", 95,
                                     f_maize_crop_size))))))))
...
table(Data$f_sorghum_measurement_lost)
table(Data$f_sorghum_own_consumption_measurement)
...
##Measurement unit for sales focus crop
  ...
  mutate(cal_sorghum_measurement_sold := ifelse(grepl("[[:digit:]]", f_sorghum_measurement_sold), 
...
# measurement unit for lost
  mutate(cal_maize_lost_measurement := ifelse(grepl("[[:digit:]]", f_maize_measurement_lost), 
    extract_numeric(f_maize_measurement_lost), 
    ifelse(f_maize_measurement_lost %in%c("kg","kgs"), 1, 0))) %>%
  
  mutate(cal_sorghum_lost_measurement := ifelse(grepl("[[:digit:]]", f_sorghum_measurement_lost), 
...
Data <- Data %>%
# measurement unit for lost
  mutate(cal_maize_measurement_own_consumption := ifelse(grepl("[[:digit:]]", f_maize_own_consumption_measurement), 
    extract_numeric(f_maize_own_consumption_measurement), 
    ifelse(f_maize_own_consumption_measurement %in%c("kg","kgs"), 1, 0))) %>%
  
   mutate(cal_sorghum_measurement_own_consumption := ifelse(grepl("[[:digit:]]", f_sorghum_own_consumption_measurement), 
...
Data <- Data %>%
  ...
  mutate(cal_maize_own_consumption_kg = f_maize_own_consumption * cal_maize_measurement_own_consumption)%>% 
#sorghum
    ...
    mutate(cal_sorghum_own_consumption_kg = f_sorghum_own_consumption * cal_sorghum_measurement_own_consumption) %>%
...
Data <- Data %>%
  ...
  ##sorghum
     mutate(cal_sorghum_price = ifelse(cal_sorghum_measurement_prod != 0, f_sorghum_price / cal_sorghum_measurement_sold, f_sorghum_price),
         cal_sorghum_price = ifelse(is.na(cal_sorghum_quant_prod_kg) | cal_sorghum_quant_prod_kg == 0, NA, cal_sorghum_price))
...
maize_quantities_own_consumption_subset <- Data %>% select(f_maize_own_consumption_measurement, cal_maize_measurement_own_consumption, f_maize_own_consumption, cal_maize_own_consumption_kg)
...
sorghum_quantities_own_consumption_subset <- Data %>% select(f_sorghum_own_consumption_measurement, cal_sorghum_measurement_own_consumption, f_sorghum_own_consumption, cal_sorghum_own_consumption_kg)
...
#
    cal_maize_quant_own_consumption_kg_season_1 = ifelse(f_maize_rev_timeperiod == "season 1 march  july", cal_maize_own_consumption_kg, NA),
    cal_maize_quant_own_consumption_kg_season_2 = ifelse(f_maize_rev_timeperiod == "season 2  august  february", cal_maize_own_consumption_kg, NA),
    cal_maize_quant_own_consumption_kg_whole_year = ifelse(f_maize_rev_timeperiod == "past 12 months whole year", cal_maize_own_consumption_kg, NA),
#
...
Data <- Data %>%
  ...
  group_by(identifier) %>%
  mutate(
    cal_maize_quant_prod_kg_season_1 = sum(na.omit(cal_maize_quant_prod_kg_season_2), na.rm = TRUE),
...
Data <- Data %>%
  # Create new columns based on the season specified in f_focus_rev_timeperiod
  mutate(
    cal_sorghum_quant_prod_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1 march  july", cal_sorghum_quant_prod_kg, NA),
    cal_sorghum_quant_prod_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2  august  february", cal_sorghum_quant_prod_kg, NA),
    ...
    #   
    cal_sorghum_quant_sold_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1 march  july", cal_sorghum_quant_sold_kg, NA),
    cal_sorghum_quant_sold_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2  august  february", cal_sorghum_quant_sold_kg, NA),
...
#
    cal_sorghum_quant_own_consumption_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1 march  july", cal_sorghum_own_consumption_kg, NA),
    cal_sorghum_quant_own_consumption_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2  august  february", cal_sorghum_own_consumption_kg, NA),
    cal_sorghum_quant_own_consumption_kg_whole_year = ifelse(f_sorghum_rev_timeperiod == "past 12 months whole year", cal_sorghum_own_consumption_kg, NA),
#
    cal_sorghum_quant_lost_kg_season_1 = ifelse(f_sorghum_rev_timeperiod == "season 1 march  july", cal_sorghum_quant_lost_kg, NA),
    cal_sorghum_quant_lost_kg_season_2 = ifelse(f_sorghum_rev_timeperiod == "season 2  august  february", cal_sorghum_quant_lost_kg, NA),
...
Data <- Data %>%
  ...
  group_by(identifier) %>%
  mutate(
    cal_sorghum_quant_prod_kg_season_1 = sum(na.omit(cal_sorghum_quant_prod_kg_season_2), na.rm = TRUE),
  ...
  mutate(cal_sorghum_own_consumption_kg = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_sorghum_own_consumption_kg )) %>%
  ...
  mutate(cal_sorghum_own_consumption_kg =  sum(cal_sorghum_own_consumption_kg)) %>%
  ...
  mutate(cal_sorghum_own_consumption_kg = ifelse(`repeat no` >1  , NA, cal_sorghum_own_consumption_kg )) %>%
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(cal_focus_revenue=cal_sorghum_revenue+cal_maize_revenue)
#subset
...
#cost per season
Data <- Data %>%
  rename(f_inputs_usage_costs_types=f_inputs_costs_types)%>%
...
@missing_f_othermaincrop_2_price
```{r, include = FALSE}
...
subset <- Data %>% 
  select(cal_maize_quant_prod_kg, cal_sorghum_quant_prod_kg, cal_maize_quant_sold_kg, cal_sorghum_quant_sold_kg, cal_maize_price, cal_sorghum_price,
         cal_maize_own_consumption_kg, cal_sorghum_own_consumption_kg, cal_maize_quant_lost_kg, cal_sorghum_quant_lost_kg,
         cal_maize_revenue, cal_sorghum_revenue, cal_maize_productivity_acre, cal_sorghum_productivity_acre,
         cal_labour_cost, 
         cal_inputs_costs, 
         cal_focus_other_costs, 
...
```{r, include = FALSE}
Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_county_) %>%  ### the variable on the right differs from case to case!!!
          select(
...
```{r}
baseline_farmers <- read_excel("pi_2022_afrokai_maize_sorghum.xlsx")
