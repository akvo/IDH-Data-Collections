...
```{r}
data_filename <- "DATA_ANALYSIS-631020070-AfroKai-potatoes.xlsx"
survey_filename <- "SURVEY_FORM-631020070-AfroKai-potatoes-merged.xlsx"
data_additional_name <- "DATA_ANALYSIS-Afrokai - Endline questions-potatoes.xlsx"
```
...
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
Data <- Data %>%
  mutate(f_focus_measurement_prod = ifelse(f_focus_measurement_prod=="", "kgs", f_focus_measurement_prod))
```
...
subset <- Data %>% 
  select(cal_focus_quant_prod_kg, cal_focus_quant_sold_kg, cal_focus_price, 
         ...
         cal_focus_revenue, cal_focus_productivity_acre,
         cal_labour_cost, 
         cal_inputs_costs, 
         cal_focus_other_costs, 
...
for (j in multiple_mc) {
  ...
  if (j == "f_labour_nurserymaint_labour_type") {
    next
  }
  ...
  if (j == "f_labour_fertilizerapp_labour_type") {
    next
  }
  if (j == "f_labour_harvesting_labour_type") {
    next
  }
  if (j == "f_labour_postharvest_labour_type") {
    next
  }
  if (j == "cs_negative_recommendation") {
    next
  }
  if (j == "hh_loan_one_agri_use") {
    next
  }
  ...
  if (j == "hh_loan_two_purpose") {
    next
  }
  if (j == "hh_loan_mulitple_agri_purpose") {
    next
  }
  if (j == "fo_decrease_land_reason") {
    next
  }
...
```{r}
baseline_farmers <- read_excel("pi_2022_afrokai_potatoes.xlsx")
