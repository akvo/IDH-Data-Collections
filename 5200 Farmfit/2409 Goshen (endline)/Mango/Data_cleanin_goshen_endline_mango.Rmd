...
```{r}
data_filename <- "DATA_ANALYSIS-641400197-goshen-mango-2024.xlsx"
survey_filename <- "SURVEY_FORM-641400197-goshen-mango-2024.xlsx"
```
...
specific adjustment for farm size dedicated to crop
```{r, include = FALSE}
Data <- Data %>%
  mutate(f_unit_land=ifelse(is.na(f_unit_land) & f_size >0, "acres", f_unit_land))

Data <- Data %>%
  mutate(f_focus_crop_size = ifelse(identifier=="6x0441x9r5n3", NA, 
                                    ifelse(f_focus_crop_size > f_size, f_focus_crop_size/10,
                                           f_focus_crop_size)))
```
...
# mutate(f_size_acre = ifelse(f_unit_land == "poles", f_size / 160, f_size)) %>%
  mutate(f_size_acre = ifelse(f_unit_land == "squared meters", f_size / 4047, f_size)) %>%
 ...
 #mutate(f_focus_crop_size_acre = ifelse(f_unit_land == "poles", f_focus_crop_size / 160, f_focus_crop_size)) %>%
  mutate(f_focus_crop_size_acre = ifelse(f_unit_land == "squared meters", f_focus_crop_size / 4047, f_focus_crop_size)) %>%
...
#Mutate the different units to hectares. We need a farm size in hectares for the total farm size, and the size dedicated to focus crop.
 ...
 #mutate(f_size_hectare = ifelse(f_unit_land == "poles", f_size / 395.4, f_size_acre / 2.471)) %>%
  mutate(f_size_hectare = ifelse(f_unit_land == "squared meters", f_size / 10000, f_size_acre / 2.471)) %>%
 mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "hectares", f_focus_crop_size, f_focus_crop_size_acre / 2.471))%>%
 mutate(f_focus_crop_size_hectare = ifelse(f_unit_land == "squared meters", f_focus_crop_size / 10000 , f_focus_crop_size_acre / 2.471))
```
...
```{r}
# Delete row with wrong data
Data <- Data %>%
  filter(identifier!="x8rw8tjcqpny")
...
Data <- Data %>%
  mutate(
    #prod
    f_focus_measurement_prod = ifelse(identifier == "j2utv6my2hee", "pieces", 
                                      ifelse(f_focus_measurement_prod == "per piece", "pieces", f_focus_measurement_prod)))
    #sold
    # f_focus_measurement_sold = ifelse(f_focus_measurement_sold == "kgs", "100 kgs bags", f_focus_measurement_sold),
    ...
    # #own consumption
    # f_focus_own_consumption_measurement =ifelse(f_focus_own_consumption_measurement == "kgs", "100 kgs bags",
    #                                             ifelse(f_focus_own_consumption_measurement == "100 kilogram bags", "100 kgs bags",
    #                                                           f_focus_own_consumption_measurement)),

    #lost, let's harmonise measurement unit 
  #   f_focus_measurement_lost =ifelse(f_focus_measurement_lost == "kgs", "100 kgs bags",
  #                                    ifelse(f_focus_measurement_lost == "100 kilogram bags", "100 kgs bags",
...
```{r}
#if all have the same unit, we can use this code to replace quantities sold by quantity prod or vice versa after checking
Data <- Data %>%
  mutate(f_focus_quant_prod = 
           ifelse(f_focus_measurement_prod == "per net", f_focus_quant_prod * 8, f_focus_quant_prod),
         f_focus_quant_sold = 
           ifelse(f_focus_measurement_sold == "per net", f_focus_quant_sold * 8, f_focus_quant_sold),
         f_focus_price=
           ifelse(f_focus_measurement_sold == "per net", f_focus_price / 8, f_focus_price),
         f_focus_measurement_prod=
           ifelse(f_focus_measurement_prod == "per net", "pieces", f_focus_measurement_prod),
         f_focus_measurement_sold=
           ifelse(f_focus_measurement_sold == "per net", "pieces", f_focus_measurement_sold)
         )

#odd quantity prod to be replaced by sold
Data <- Data %>%
  mutate(f_focus_quant_prod =ifelse(identifier== "sywp57v7snda", f_focus_quant_sold, f_focus_quant_prod))

#some reported revenue in the price column and price in th quantity sold.
identifier_to_replace <- c("up3n024b5s76", "ys65xbuaunt5", "an09xq7vb91h", "t14tsue0seam")
Data <- Data %>%
  mutate(price_for_replace=ifelse(identifier %in% identifier_to_replace, f_focus_quant_sold, 0),
         f_focus_quant_sold = ifelse(identifier %in% identifier_to_replace, f_focus_price/f_focus_quant_sold, f_focus_quant_sold),
         f_focus_price = ifelse(identifier %in% identifier_to_replace, price_for_replace, f_focus_price))


#quantity produced lower than sold, do sold=prod if they have the same unit
Data <- Data %>%
  mutate(f_focus_quant_sold = ifelse(f_focus_quant_sold > f_focus_quant_prod & f_focus_measurement_sold == f_focus_measurement_prod, f_focus_quant_prod, f_focus_quant_sold))

#Did not use the same unit for prod and sold but sold still higher than prod after conversion. replace prod = sold or vice versa regarding the farm size
Data <- Data %>%
  mutate(f_focus_quant_prod= ifelse(identifier %in% c("wva21jkv9e46", "mcf7wwsugctj"), f_focus_quant_sold, f_focus_quant_prod),
         f_focus_measurement_prod= ifelse(identifier == "wva21jkv9e46", f_focus_measurement_sold, f_focus_measurement_prod))
# Data <- Data %>%
#   mutate(f_focus_measurement_sold= ifelse(identifier == "jr7u8mmxvbeb", NA_character_ , f_focus_measurement_sold))
...
Data <- Data %>%
  mutate(f_focus_measurement_prod = ifelse(f_focus_quant_prod== 0 | is.na(f_focus_quant_prod), NA_character_ , f_focus_measurement_prod),
         f_focus_measurement_sold = ifelse(f_focus_quant_sold== 0 | is.na(f_focus_quant_sold), NA_character_ , f_focus_measurement_sold),
         f_focus_measurement_lost = ifelse(f_focus_quant_lost== 0 | is.na(f_focus_quant_lost), NA_character_ , f_focus_measurement_lost),
         f_focus_own_consumption_measurement = ifelse(f_focus_own_consumption == 0 | is.na(f_focus_own_consumption), NA_character_ , f_focus_own_consumption_measurement)
  )
```
...
#some need to be multiplied by 100 to be in line with the sold unit
Data <- Data %>%
  mutate(f_focus_price = ifelse(is.na(f_focus_quant_prod) & is.na(f_focus_quant_sold), NA, f_focus_price))

#calculate median
median_price_kg<- Data %>%
   filter(f_focus_measurement_sold == "kgs") %>%
   summarize(median_price_kg = median(f_focus_price, na.rm = TRUE)) 

median_price_piece<- Data %>%
   filter(f_focus_measurement_sold == "pieces") %>%
   summarize(median_price_piece = median(f_focus_price, na.rm = TRUE))

 Data <- Data %>%
  mutate(f_focus_price = ifelse(f_focus_measurement_sold == "pieces" & (f_focus_price <5 | f_focus_price > 10), median_price_kg$median_price_kg,
                                ifelse(f_focus_measurement_sold == "kgs" & (f_focus_price <11 | f_focus_price > 22), median_price_piece$median_price_piece,
                                f_focus_price)))
...
Rewriting the quantities deleted by the outlier function
```{r}
Data <- Data %>%
  mutate(f_size = ifelse(identifier == "4b2q9taf65am" & f_size_acre!=0, 8,
                         ifelse(identifier == "rcvjy55d167w"& f_size_acre!=0, 10,
                                ifelse(identifier == "7ekjggnsr09u" & f_size_acre!=0, 9,
                                       ifelse(identifier == "qy61bnjd877a" & f_size_acre!=0, 8,
                                              ifelse(identifier == "evw3es1r8ra5" & f_size_acre!=0, 10,
                                       f_size))))),
         f_focus_crop_size = ifelse(identifier == "qy61bnjd877a" & f_focus_crop_size_acre!=0, 4,
                         ifelse(identifier == "wva21jkv9e46" & f_focus_crop_size_acre!=0, 4,
                                ifelse(identifier == "cpwprq3rj6tb" & f_focus_crop_size_acre!=0, 5,
                                       ifelse(identifier == "6x0441x9r5n3" & f_focus_crop_size_acre!=0, 0.5,
                                              ifelse(identifier == "yhtwrgmfvhjp" & f_focus_crop_size_acre!=0, 5,
                                    f_focus_crop_size)))))
         )


Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier == "mxcf35uq5nye" & is.na(f_focus_quant_prod), 40000,
                                    ifelse(identifier == "pmr7ku7qrbu6" & is.na(f_focus_quant_prod), 30000,
                                           ifelse(identifier == "bcght9n8bu43" & is.na(f_focus_quant_prod), 30000,
                                                  ifelse(identifier == "m898qjv665w9" & is.na(f_focus_quant_prod), 30000,
                                                         ifelse(identifier == "tj506qufqc6y" & is.na(f_focus_quant_prod), 40000,
                                                                ifelse(identifier == "9475ektd8xx3" & is.na(f_focus_quant_prod), 25000,
                                    f_focus_quant_prod)))))),
         f_focus_quant_sold = ifelse(identifier == "pmr7ku7qrbu6" & is.na(f_focus_quant_sold), 30000,
                                    ifelse(identifier == "bcght9n8bu43" & is.na(f_focus_quant_sold), 29700,
                                                  ifelse(identifier == "m898qjv665w9" & is.na(f_focus_quant_sold), 27000,
                                                         ifelse(identifier == "9475ektd8xx3" & is.na(f_focus_quant_sold), 25000,
                                    f_focus_quant_sold))))
  )

Data <- Data %>%
  mutate(f_focus_price = ifelse(identifier== "4rnkacs1sgkg" & is.na(f_focus_price), 19,
                               ifelse(identifier== "kjsj4km8ju0a" & is.na(f_focus_price), 19, f_focus_price)))

Data <- Data %>%
  mutate(f_focus_price= ifelse(is.na(f_focus_quant_sold), NA, f_focus_price),
         f_focus_measurement_prod=ifelse(is.na(f_focus_quant_prod), NA_character_, f_focus_measurement_prod),
         f_focus_measurement_sold=ifelse(is.na(f_focus_quant_sold), NA_character_, f_focus_measurement_sold))
```
...
##Measurement unit for production
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      ...
      ifelse(f_focus_measurement_prod %in%c("kg", "kgs", "pieces", "per piece", "per net"), 1, 0)))
```
...
##Measurement unit for sales focus crop
  mutate(cal_focus_measurement_sold := ifelse(
      ...
      ifelse(f_focus_measurement_sold %in%c("kg", "kgs", "pieces", "per piece", "per net"), 1, 0)))
...
# measurement unit for lost
  mutate(cal_focus_lost_measurement := ifelse(
    ...
    ifelse(f_focus_measurement_lost %in%c("kg", "kgs", "pieces", "per piece", "per net"), 1, 0)))
```
...
# measurement unit for lost
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in%c("kg", "kgs", "pieces", "per piece", "per net"), 1, 0))) 
```
...
Data <- Data %>%
  mutate(cal_focus_quant_prod_kg = ifelse(f_focus_measurement_prod !="pieces", f_focus_quant_prod * cal_focus_measurement_prod, 0),
         cal_focus_quant_prod_piece = ifelse(f_focus_measurement_prod =="pieces", f_focus_quant_prod * cal_focus_measurement_prod, 0))%>%
  
  mutate(cal_focus_quant_sold_kg = ifelse(f_focus_measurement_sold !="pieces", f_focus_quant_sold * cal_focus_measurement_sold, 0),
         cal_focus_quant_sold_piece = ifelse(f_focus_measurement_sold =="pieces", f_focus_quant_sold * cal_focus_measurement_sold, 0))%>%
  
  mutate(cal_focus_quant_lost_kg = ifelse(f_focus_measurement_lost !="pieces", f_focus_quant_lost * cal_focus_lost_measurement, 0),
         cal_focus_quant_lost_piece = ifelse(f_focus_measurement_lost =="pieces", f_focus_quant_lost * cal_focus_lost_measurement, 0))%>%
  
  mutate(cal_focus_quant_own_consumption_kg = ifelse(f_focus_own_consumption_measurement !="pieces", f_focus_own_consumption * cal_focus_measurement_own_consumption, 0),
         cal_focus_quant_own_consumption_piece = ifelse(f_focus_own_consumption_measurement =="pieces", f_focus_own_consumption * cal_focus_measurement_own_consumption, 0))%>%
  
  mutate_at(vars(starts_with("cal_focus_quant")), ~replace_na(., 0)) 
    
  # also calculate the unit price 
Data <- Data %>%
  mutate(cal_focus_price_kg = ifelse(cal_focus_measurement_prod != 0 & f_focus_measurement_sold !="pieces", f_focus_price/cal_focus_measurement_sold, 0),
         cal_focus_price_piece = ifelse(cal_focus_measurement_prod != 0 & f_focus_measurement_sold =="pieces", f_focus_price/cal_focus_measurement_sold, 0),
         
    cal_focus_price_kg = ifelse(is.na(cal_focus_quant_sold_kg) | cal_focus_quant_sold_kg == 0, NA, cal_focus_price_kg),
    cal_focus_price_piece = ifelse(is.na(cal_focus_quant_sold_piece) | cal_focus_quant_sold_piece == 0, NA, cal_focus_price_piece))%>%
  mutate(cal_focus_price_kg = replace_na(cal_focus_price_kg, 0),
         cal_focus_price_piece = replace_na(cal_focus_price_piece, 0))
```
...
quantities_produced_subset <- Data %>% select(f_focus_measurement_prod, cal_focus_measurement_prod, f_focus_quant_prod, cal_focus_quant_prod_kg, cal_focus_quant_prod_piece)

quantities_sold_subset <- Data %>% select(f_focus_measurement_sold, cal_focus_measurement_sold, f_focus_quant_sold, cal_focus_quant_sold_kg, cal_focus_quant_sold_piece)

quantities_lost_subset <- Data %>% select(f_focus_measurement_lost, cal_focus_lost_measurement, f_focus_quant_lost, cal_focus_quant_lost_kg, cal_focus_quant_lost_piece)

quantities_own_consumption_subset <- Data %>% select(f_focus_own_consumption_measurement, cal_focus_measurement_own_consumption, f_focus_own_consumption, cal_focus_quant_own_consumption_kg, cal_focus_quant_own_consumption_piece)

price_subset <- Data %>% select(f_focus_price, cal_focus_measurement_prod, cal_focus_price_kg, cal_focus_price_piece)
```
...
```{r}
Data <- Data %>%
  mutate(cal_focus_price_kg = ifelse(cal_focus_quant_sold_kg > 0 & is.na(cal_focus_price_kg), median(cal_focus_price_kg), cal_focus_price_kg),
         cal_focus_price_piece = ifelse(cal_focus_quant_sold_piece > 0 & is.na(cal_focus_price_piece), median(cal_focus_price_piece), cal_focus_price_piece))
```
...
```{r, include = FALSE}
Data <- Data %>%
  mutate(cal_focus_revenue = cal_focus_quant_sold_kg * cal_focus_price_kg + cal_focus_quant_sold_piece * cal_focus_price_piece) 
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_prod_piece_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_prod_piece, NA),
    cal_focus_quant_prod_piece_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_prod_piece, NA),
    cal_focus_quant_prod_piece_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_prod_piece, NA),
    #   
    ...
    cal_focus_quant_sold_piece_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_sold_piece, NA),
    cal_focus_quant_sold_piece_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_sold_piece, NA),
    cal_focus_quant_sold_piece_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_sold_piece, NA),
#
    cal_focus_quant_own_consumption_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_own_consumption_kg, NA),
    cal_focus_quant_own_consumption_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_own_consumption_kg, NA),
    cal_focus_quant_own_consumption_kg_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_own_consumption_kg, NA),
cal_focus_quant_own_consumption_piece_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_own_consumption_piece, NA),
    cal_focus_quant_own_consumption_piece_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_own_consumption_piece, NA),
    cal_focus_quant_own_consumption_piece_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_own_consumption_piece, NA),
#
...
cal_focus_quant_lost_piece_season_1 = ifelse(f_focus_rev_timeperiod == "season 1", cal_focus_quant_lost_piece, NA),
    cal_focus_quant_lost_piece_season_2 = ifelse(f_focus_rev_timeperiod == "season 2", cal_focus_quant_lost_piece, NA),
    cal_focus_quant_lost_piece_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_lost_piece, NA)
  )
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_prod_piece_season_1 = sum(na.omit(cal_focus_quant_prod_piece_season_1), na.rm = TRUE),
    cal_focus_quant_prod_piece_season_2 = sum(na.omit(cal_focus_quant_prod_piece_season_2), na.rm = TRUE),
    cal_focus_quant_prod_piece_whole_year = sum(na.omit(cal_focus_quant_prod_piece_whole_year), na.rm = TRUE),
    cal_focus_quant_sold_piece_season_1 = sum(na.omit(cal_focus_quant_sold_piece_season_1), na.rm = TRUE),
    cal_focus_quant_sold_piece_season_2 = sum(na.omit(cal_focus_quant_sold_piece_season_2), na.rm = TRUE),
    cal_focus_quant_sold_piece_whole_year = sum(na.omit(cal_focus_quant_sold_piece_whole_year), na.rm = TRUE), 
    cal_focus_quant_own_consumption_piece_season_1 = sum(na.omit(cal_focus_quant_own_consumption_piece_season_1), na.rm = TRUE),
    cal_focus_quant_own_consumption_piece_season_2 = sum(na.omit(cal_focus_quant_own_consumption_piece_season_2), na.rm = TRUE),
    cal_focus_quant_own_consumption_piece_whole_year = sum(na.omit(cal_focus_quant_own_consumption_piece_whole_year), na.rm = TRUE),
    cal_focus_quant_lost_piece_season_1 = sum(na.omit(cal_focus_quant_lost_piece_season_1), na.rm = TRUE),
    cal_focus_quant_lost_piece_season_2 = sum(na.omit(cal_focus_quant_lost_piece_season_2), na.rm = TRUE),
    cal_focus_quant_lost_piece_whole_year = sum(na.omit(cal_focus_quant_lost_piece_whole_year), na.rm = TRUE)
  ) %>% 
  ...
  mutate(cal_focus_quant_prod_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_prod_piece)) %>%
  mutate(cal_focus_quant_sold_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_sold_piece)) %>%
  mutate(cal_focus_quant_own_consumption_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_own_consumption_piece )) %>%
  mutate(cal_focus_quant_lost_piece = ifelse(`repeat no` >1  & count_dup > 1, 0, cal_focus_quant_lost_piece)) %>%
  ...
  mutate(cal_focus_quant_prod_piece =  sum(cal_focus_quant_prod_piece)) %>%
  mutate(cal_focus_quant_sold_piece =  sum(cal_focus_quant_sold_piece)) %>%
  mutate(cal_focus_quant_own_consumption_piece =  sum(cal_focus_quant_own_consumption_piece)) %>%
  mutate(cal_focus_quant_lost_piece =  sum(cal_focus_quant_lost_piece)) %>%
  ...
  mutate(cal_focus_quant_prod_piece = ifelse(`repeat no` >1 , NA, cal_focus_quant_prod_piece)) %>%
  mutate(cal_focus_quant_sold_piece = ifelse(`repeat no` >1  , NA, cal_focus_quant_sold_piece)) %>%
  mutate(cal_focus_quant_own_consumption_piece = ifelse(`repeat no` >1  , NA, cal_focus_quant_own_consumption_piece )) %>%
  mutate(cal_focus_quant_lost_piece = ifelse(`repeat no` >1 , NA, cal_focus_quant_lost_piece)) %>%
...
Data <- Data %>%
  mutate(cal_focus_productivity_kg_acre = ifelse(f_focus_measurement_prod != "pieces", cal_focus_quant_prod_kg / f_focus_crop_size_acre,NA),
         cal_focus_productivity_piece_acre = ifelse(f_focus_measurement_prod == "pieces", cal_focus_quant_prod_piece/f_focus_crop_size_acre, NA))
```
...
subset <- Data %>% 
  select(cal_focus_quant_prod_kg, cal_focus_quant_prod_piece, cal_focus_quant_sold_kg, cal_focus_quant_sold_piece, 
         cal_focus_price_kg, cal_focus_price_piece,
         cal_focus_quant_own_consumption_kg, cal_focus_quant_own_consumption_piece, 
         cal_focus_quant_lost_kg, cal_focus_quant_lost_piece,
         cal_focus_revenue, cal_focus_productivity_kg_acre, cal_focus_productivity_piece_acre, cal_labour_cost, cal_inputs_costs, 
         cal_focus_other_costs, cal_focus_cost, cal_focus_income, cal_othermaincrop_1_cost, cal_othermaincrop_2_cost, #cal_othermaincrop_production_value,
         cal_othermaincrop_1_inc_sold, cal_othermaincrop_2_inc_sold,cal_other_crop_income, cal_livestock_income,
         cal_equipment_costs,#cal_farm_costs_loans, 
...
#  Skip the variables with no data
...
NB:If the pi_ file is not available for baseline, use the raw data and generate it. here is an example
```{r}
...
```{r}
baseline_farmers <- read_excel("pi_2022_goshen_mango.xlsx")
...
productivity <- Data %>%
    mutate(focus_product_min_sold_kg = cal_focus_quant_prod_kg - cal_focus_quant_sold_kg,
           focus_product_min_sold_piece = cal_focus_quant_prod_piece - cal_focus_quant_sold_piece) %>%
 ...
 #          submitter,
           ...
           cal_focus_productivity_piece_acre,
           f_focus_quant_prod,
           f_focus_measurement_prod,
           cal_focus_quant_prod_kg,
           cal_focus_quant_prod_piece,
           f_focus_quant_sold,
           f_focus_measurement_sold,
           cal_focus_quant_sold_kg,
           cal_focus_quant_sold_piece,
