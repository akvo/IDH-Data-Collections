...
```{r}
data_filename <- "DATA_ANALYSIS-645010220-goshen-pineapple-2024.xlsx"
survey_filename <- "SURVEY_FORM-645010220-goshen-pineapple-2024.xlsx"
```
...
```{r}
Data <- Data %>%
  mutate(f_focus_crop_size = ifelse(f_size < f_focus_crop_size, f_size, f_focus_crop_size))
...
Data <- Data %>%
  mutate(f_focus_quant_sold = 
           ifelse(f_focus_quant_prod < f_focus_quant_sold, f_focus_quant_prod, f_focus_quant_sold))

#odd quantities
 # identifier_to_replace <- c("wqrvv7ph6ecy", "p93vjvhwrcdf", "pun4r658t1q9", "7056gng9vsy3", "ge42u73kqgs5", "trvc6a1khj6h", "mfaxvhkgf0b8", "u1dbg188nj0d", "ndv1eyefppwg")

 Data <- Data %>%
   mutate(f_focus_quant_prod = ifelse(f_focus_quant_prod>100000, f_focus_quant_prod / 10, 
                                                          f_focus_quant_prod))
 
         #      ifelse(f_focus_quant_sold > f_focus_quant_prod, f_focus_quant_prod, f_focus_quant_sold)),
         #  f_focus_own_consumption=ifelse(identifier=="8w6w9wt0u2km",f_focus_own_consumption / 100, f_focus_own_consumption),
         # f_focus_quant_lost=ifelse(identifier=="xgqx52sdaagn", f_focus_quant_lost / 100, f_focus_quant_lost))
...
#calculate median to replace odd prices
 # median_price<- Data %>%
...
#calculate median
 ...
 Data <- Data %>%
  mutate(f_focus_price = ifelse((f_focus_price <0 | f_focus_price > 70) & f_focus_measurement_sold == "pieces", median_price_piece$median_price_piece, f_focus_price))
...
```{r}
Data <- Data %>% filter(!(identifier=="utbvs3scg2wf" & `repeat no` == 3))
```
...
```{r}
Data <- Data %>%
  mutate(f_size = ifelse(identifier == "nw4s96d6bbx5", 48,
                         ifelse(identifier == "u1dbg188nj0d", 42,
                                ifelse(identifier == "ndv1eyefppwg", 42,
                                       f_size))),
         f_focus_crop_size = ifelse(identifier == "rryf1njr1epb", 7,
                                    f_focus_crop_size)
         )


 Data <- Data %>%
   mutate(f_focus_quant_prod = ifelse(identifier == "u1dbg188nj0d", 96390,
                                     f_focus_quant_prod),
          f_focus_quant_lost = ifelse(identifier == "pun4r658t1q9", 27000,
                                     f_focus_quant_lost)
          )
```
...
##Measurement unit for production
  mutate(cal_focus_measurement_prod := ifelse(grepl("[[:digit:]]", f_focus_measurement_prod), 
      ...
      ifelse(f_focus_measurement_prod %in%c("kg", "kgs", "pieces"), 1, 0)))
```
...
##Measurement unit for sales focus crop
  mutate(cal_focus_measurement_sold := ifelse(
      ...
      ifelse(f_focus_measurement_sold %in%c("kg", "kgs", "pieces"), 1, 0)))
...
# measurement unit for lost
  mutate(cal_focus_lost_measurement := ifelse(
    ...
    ifelse(f_focus_measurement_lost %in%c("kg", "kgs", "pieces"), 1, 0)))
```
...
# measurement unit for lost
  mutate(cal_focus_measurement_own_consumption := ifelse(
    ...
    ifelse(f_focus_own_consumption_measurement %in%c("kg", "kgs", "pieces"), 1, 0))) 
```
...
```{r, include = FALSE}
#calculate the revenue (sum of the two revenues. ensure to replace NAs by 0 before calculating the global revenue)
Data <- Data %>%
...
```{r}
baseline_farmers <- read_excel("pi_2022_goshen_pineapple.xlsx")
