...
for(i in 1:length(cases)){
  ...
  # Changes identified by going through the lists BY HAND! 
...
for(i in 1:length(cases)){
    ...
    if(cases[i] == "kenya-syngenta_potatoes.xlsx"){
      case <- case %>% rename(
        ...
        f_inputs_source_other = f_inputs_source_1,
        f_inputs_challenges_other = f_inputs_challenges_1,
        f_inputs_challenges_types_other = f_inputs_challenges_types)
    }
  }
    ############################
...
#Export new versions
write.xlsx(case, file = here("/Volumes/GoogleDrive/My Drive/04_data_analytics/Farmfit data alignment/Sampage/Original/", 
...
write.xlsx(codebook, file = here("/Volumes/GoogleDrive/My Drive/04_data_analytics/Farmfit data alignment/Sampage/Original/", 
                                 "2021-05-26_EU Tea Tanzania_anom_Njombe_Ikanga_farmers.xlsx"),
           sep=",", 
           append = TRUE,
           row.names = FALSE)
