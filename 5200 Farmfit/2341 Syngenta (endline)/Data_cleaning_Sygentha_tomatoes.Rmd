...
```{r}
data_filename <- "DATA_ANALYSIS_Syngenta_tomatoes.xlsx"
survey_filename <- "SURVEY_FORM_Syngenta_tomatoes.xlsx"
```
...
```{r}
# THIS IS EXAMPLE CODE! ADAPT THE CODE TO THE NECESSARY ADJUSTMENTS IN THE CASE, OR DELETE CODE IF NO CLEANING IS REQUIRED
#we noticed that large quantities in kgs are deleted with the outlier function. we reconvert them like 30000kgs==quant 300 and unit 100kg. then we replace the price by median price of 100kg

median_price_100kg<-Data %>%
  filter(f_focus_measurement_sold == "100kg") %>%
  summarize(median_price_100kg = median(f_focus_price, na.rm = TRUE))

identifiers_to_replace <- c("my0nt9ec48yg","pnwnkdhtux1w","ug5rstfrc37e","f17ue9x630ey","xg5rpf126yne","v0u0s3crusge")
#now replace
Data <- Data %>%
  mutate(f_focus_price = ifelse(
    identifier %in% identifiers_to_replace & !is.na(median_price_100kg$median_price_100kg),
    median_price_100kg$median_price_100kg,
    f_focus_price
  ))
...
Data <- Data %>%
  mutate(
    f_focus_measurement_prod = case_when(
      identifier %in% c("21rwcemdsbhb","nuq5cn8vcu8y","8vttwba92je8") ~ "crate 60 kg",
      identifier %in% c("2ywx4d1kduvs", "31tkxxpmygdp","my0nt9ec48yg","pnwnkdhtux1w","ug5rstfrc37e","f17ue9x630ey","xg5rpf126yne","v0u0s3crusge") ~ "100kg",
       identifier %in% c("n148gxhmws1v", "n148gxhmws1v") ~ "120kg crate",
      identifier %in% c("6kurjejyfdq2", "af027f0cp4mt", "aqya1h7e0cc4","8637gdnfexpb","026cneq7djwt","myybeds9tpm0") ~ NA_character_,
      TRUE ~ f_focus_measurement_prod
    ),
    f_focus_measurement_sold = case_when(
      identifier %in% c("2ywx4d1kduvs", "31tkxxpmygdp","my0nt9ec48yg","pnwnkdhtux1w","ug5rstfrc37e","f17ue9x630ey","xg5rpf126yne","v0u0s3crusge") ~ "100kg",
      identifier=="r6ys4q1xk6e2" ~ "80kg",
      identifier %in% c("026cneq7djwt", "6kurjejyfdq2", "8637gdnfexpb","af027f0cp4mt","aqya1h7e0cc4","myybeds9tpm0","vewb4adwda3y") ~ NA_character_,
      TRUE ~ f_focus_measurement_sold
    ),
    f_focus_measurement_lost=ifelse(identifier=="2ywx4d1kduvs", "100kg", f_focus_measurement_lost)
  )
```
...
Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier %in% c("6kurjejyfdq2", "af027f0cp4mt", "aqya1h7e0cc4","8637gdnfexpb","026cneq7djwt","myybeds9tpm0"), NA,
                                     ifelse(identifier == "31tkxxpmygdp", 8,
                                            ifelse(identifier == "pnwnkdhtux1w", 450,
                                            ifelse(identifier == "ug5rstfrc37e", 100,
                                            ifelse(identifier %in% c("f17ue9x630ey","v0u0s3crusge"), 50,
                                            ifelse(identifier == "xg5rpf126yne", 80,
                                            ifelse(identifier %in% c("k69wnk8dvrh2","mttbk62w9t1w","my0nt9ec48yg"), 150, f_focus_quant_prod))))))),
         f_focus_quant_sold = ifelse(identifier %in% c("026cneq7djwt", "6kurjejyfdq2", "8637gdnfexpb","af027f0cp4mt","aqya1h7e0cc4","myybeds9tpm0","vewb4adwda3y"), NA,
                                      ifelse(identifier == "31tkxxpmygdp", 7,
                                      ifelse(identifier == "ds644eswm7mb", 300,
                                      ifelse(identifier == "qc1c9jqhc0xg", 200,
                                      ifelse(identifier == "my0nt9ec48yg", 150,
                                      ifelse(identifier == "pnwnkdhtux1w", 430,
                                      ifelse(identifier %in% c("f17ue9x630ey","v0u0s3crusge"), 50,
                                      ifelse(identifier == "xg5rpf126yne", 80,
                                      ifelse(identifier %in% c("1rc84gvev1qs","fv0fm3kqvu25","ug5rstfrc37e"), 100,
                                      ifelse(identifier == "vg8v8s3s6wgf", 400, f_focus_quant_sold))))))))))
)
```
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_own_consumption_kg_season_3 = ifelse(f_focus_rev_timeperiod == "season 3", cal_focus_measurement_own_consumption, NA),
...
Data <- Data %>%
  ...
  mutate(cal_focus_quant_prod_kg = ifelse(`repeat no` >1 , 0, cal_focus_quant_prod_kg)) %>%
  mutate(cal_focus_quant_sold_kg = ifelse(`repeat no` >1  , 0, cal_focus_quant_sold_kg)) %>%
  mutate(cal_focus_quant_own_consumption_kg = ifelse(`repeat no` >1  , 0, cal_focus_quant_own_consumption_kg )) %>%
  mutate(cal_focus_quant_lost_kg = ifelse(`repeat no` >1 , 0, cal_focus_quant_lost_kg)) %>%
  mutate(cal_focus_revenue = ifelse (`repeat no`>1, 0, cal_focus_revenue)) %>%
  ungroup() %>%
...
Data <- Data %>% mutate(cal_labour_cost = ifelse(is.na(cal_labour_cost), 0, cal_labour_cost))
```
...
#Ensure we have 1 row of data for each farmer
mutate(cal_labour_cost = ifelse(`repeat no` >1 , 0, cal_labour_cost)) %>%
...
group_by(identifier) %>%
  ...
  #Ensure we have 1 row of data for each farmer
  mutate(cal_inputs_costs = ifelse(`repeat no` >1 , 0, cal_inputs_costs)) %>%
  ungroup() %>%
  select(-count_dup, -count_seasons)
```
...
```{r}
table(Data$hh_loan_one_value_interest)
```
...
```{r}
baseline_farmers <- read_excel("pi_syngenta_tomatoes_baseline.xlsx")
