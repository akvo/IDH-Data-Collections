---
title: "Data_clearning_Ghana_endline"
...
```{r}
data_filename <- "DATA_ANALYSIS-654160426- Agrictrade-rice-2024.xlsx"
survey_filename <- "SURVEY_FORM-654160426-Agrictrade-rice-2024.xlsx"
```
...
Data <- Data %>%
  mutate(f_size = ifelse(f_size < f_focus_crop_size, f_focus_crop_size, f_size))
   
#all farm size should be in acres
Data <- Data %>%
...
Data <- Data %>%
  mutate(
    #prod, we have missing 1 unit and another one is "i don't know", we correct them based on farm size and quantity sold
    f_focus_measurement_prod = ifelse(identifier %in% c("qyn74spbgemf", "ga0evd39fhkx"), "100 kgs bags", f_focus_measurement_prod),
    #sold
    f_focus_measurement_sold = ifelse(identifier == "gu75635ajvnm", "100 kgs bags", f_focus_measurement_sold),
    ...
    f_focus_own_consumption_measurement =ifelse(f_focus_own_consumption_measurement == "kgs", "100 kgs bags",
                                                ifelse(f_focus_own_consumption_measurement == "100 kilogram bags", "100 kgs bags",
                                                       ifelse(f_focus_own_consumption_measurement == "" & f_focus_own_consumption != 0, "100 kgs bags",
                                                              f_focus_own_consumption_measurement))),
    f_focus_own_consumption = ifelse(f_focus_own_consumption == 9999, NA, f_focus_own_consumption),
...
Data <- Data %>%
  mutate(f_focus_quant_sold = 
           ifelse(f_focus_quant_sold > f_focus_quant_prod , f_focus_quant_prod,
              ifelse(f_focus_quant_sold == 0 & f_focus_price != 0, f_focus_quant_prod, 
                     ifelse(identifier == "u7dj6xa1jvb6", 13, f_focus_quant_sold))))
...
```{r}
Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(f_focus_quant_prod == 0, NA, f_focus_quant_prod),
         f_focus_quant_sold = ifelse(f_focus_quant_sold == 0, NA, f_focus_quant_sold),
         f_focus_own_consumption = ifelse(f_focus_own_consumption == 0, NA, f_focus_own_consumption),
         f_focus_quant_lost = ifelse(f_focus_quant_lost == 0, NA, f_focus_quant_lost),
    f_focus_measurement_prod = ifelse(is.na (f_focus_quant_prod) , NA_character_, f_focus_measurement_prod),
         ...
         f_focus_own_consumption_measurement = ifelse(is.na (f_focus_own_consumption) , NA_character_, f_focus_own_consumption_measurement),
         f_focus_measurement_lost = ifelse(is.na (f_focus_quant_lost) , NA_character_, f_focus_measurement_lost))
```
...
#calculate median
 median_price_100kgs<- Data %>%
   filter(f_focus_measurement_sold == "100 kgs bags") %>%
   summarize(median_price_100kgs = median(f_focus_price, na.rm = TRUE)) 
# 
 
 # Replace prices with median for specific identifiers with odd prices
 Data <- Data %>%
   mutate(f_focus_price = if_else(f_focus_quant_sold > 0 & f_focus_price == 0,
                                  median_price_100kgs$median_price_100kgs, f_focus_price))
...
@Warning: Specific for this case, some empty rows (mistake in repeat option) are making calcultations difficult, we delete them
```{r, include = FALSE}

Data <- Data %>%
  filter(!(identifier == "gu75635ajvnm" & `repeat no` != 1))
...
Ajustements dans quantités supprimées par outlier function
```{r}
Data <- Data %>%
  mutate(f_size = ifelse(identifier == "73ys8x1kqx7f", 14, 
                         ifelse(identifier == "92gqdkfhgp6r", 12,
                                ifelse(identifier == "trga6rtqtphm", 16,
                                       ifelse(identifier == "e0bhtynn2kw5", 12,
                                              ifelse(identifier == "m83pu039q90b", 23,
                         f_size))))),
         f_focus_crop_size = ifelse(identifier == "s2m4c43ugqs3", 5,
                                    ifelse(identifier == "trga6rtqtphm", 6,
                                           ifelse(identifier == "m83pu039q90b", 10,
                                    f_focus_crop_size))))


Data <- Data %>%
  mutate(f_focus_quant_prod = ifelse(identifier == "9mb03fckj3p5", 63,
                                    ifelse(identifier == "r292gy04nxha", 67,
                                           ifelse(identifier == "0xcrdemfc0gc", 60,
                                                  ifelse(identifier == "m83pu039q90b", 60,
                                                         ifelse(identifier == "s2m4c43ugqs3", 60,
                                                                ifelse(identifier == "92gqdkfhgp6r", 65,
                                                                       ifelse(identifier == "ktbfpry8ks1d", 60,
                                    f_focus_quant_prod))))))),
         
         f_focus_quant_sold = ifelse(identifier == "9mb03fckj3p5", 53,
                                    ifelse(identifier == "r292gy04nxha", 60,
                                                  ifelse(identifier == "0xcrdemfc0gc", 50,
                                                         ifelse(identifier == "m83pu039q90b", 60,
                                                                ifelse(identifier == "ktbfpry8ks1d", 57,
                                    f_focus_quant_sold))))),
         
         f_focus_quant_lost = ifelse(identifier == "epnf4sgqgx80", 8,
                                    f_focus_quant_lost)
         )
```
...
```{r}
#identifier = gu75635ajvnm
Data <- Data %>%
# measurement unit for prod
...
Data <- Data %>%
  ...
  mutate(
    cal_focus_quant_prod_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season one", cal_focus_quant_prod_kg, NA),
    cal_focus_quant_prod_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season two", cal_focus_quant_prod_kg, NA),
#    cal_focus_quant_prod_kg_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_prod_kg, NA), #   
    cal_focus_quant_sold_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season one", cal_focus_quant_sold_kg, NA),
    cal_focus_quant_sold_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season two", cal_focus_quant_sold_kg, NA),
    cal_focus_quant_sold_kg_whole_year = ifelse(f_focus_rev_timeperiod == "past 12 months whole year", cal_focus_quant_sold_kg, NA),
#
    cal_focus_quant_own_consumption_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season one", cal_focus_measurement_own_consumption, NA),
    cal_focus_quant_own_consumption_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season two", cal_focus_measurement_own_consumption, NA),
...
#
    cal_focus_quant_lost_kg_season_1 = ifelse(f_focus_rev_timeperiod == "season one", cal_focus_quant_lost_kg, NA),
    cal_focus_quant_lost_kg_season_2 = ifelse(f_focus_rev_timeperiod == "season two", cal_focus_quant_lost_kg, NA),
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_inputs_costs_season_2 = ifelse(f_focus_inputs_rev_timeperiod == "season 2e", cal_inputs_costs, NA)
...
```{r, include = FALSE}
variables_to_replace <- c("f_othermaincrop_1_quant_prod", 
                          #"f_othermaincrop_1_price",
"f_othermaincrop_2_quant_prod", "f_othermaincrop_2_price")

Data <- Data %>%
  mutate(f_othermaincrop_1_quant_prod=as.numeric(f_othermaincrop_1_quant_prod))%>%
  mutate_at(vars(all_of(variables_to_replace)), ~replace_na(., 0))

Data <- Data %>%
  mutate(cal_othermaincrop_production_value = 
#           f_othermaincrop_1_quant_prod * f_othermaincrop_1_price + 
           f_othermaincrop_2_quant_prod * f_othermaincrop_2_price)
```
