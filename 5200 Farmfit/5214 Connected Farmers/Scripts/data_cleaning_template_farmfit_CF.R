...
setwd("/Users/ineslesire/Documents/GitHub/Analytics-Guild/1. Descriptive Analytics/IDH/5200 Farmfit/5214 Connected Farmers")
##  ---- Describe datafiles ----
##DOWNLOAD THESE ALL TOGETHER IN A FOLDER
## Rename your files
pi_filename <- "pi_connectedfarmer.xls"
data_filename <- "Data/DATA_ANALYSIS-296820004.xlsx"
survey_filename <- "Data/SURVEY_FORM-296820004.xls"
data_delivery <- "2022_CF_anom_sunflowerseeds.xlsx"
...
# Open the file productivity_check xlsx. Check the results, also using the intake form. Check with the data collection lead why certain numbers are odd. 
  ...
  # Renamed pigs1 variable to donkey
  # survey_questions <- survey_questions %>%
  # mutate(variable = ifelse(variable == "f_livestock_income_pigs_1", "f_livestock_income_donkey", variable))
  
  rename(f_labour_cropmaint_paymentperactivity = f_labour_cropmaint_paymentpertimeframe)
  ...
  Data <- Data %>%
    ...
    mutate(cal_hh_farmer_age = 2022 - hh_farmer_birthyear,
           ...
           # cal_hh_member_birthyear = 2022-hh_member_birthyear
           ) #update to current year, check whether applicable, not always included.
...
#     #If there are other measurement units used like bucket, bags etc, use the below code
      Data <- Data %>%
     mutate(cal_focus_measurement_prod := ifelse(
       f_focus_measurement_sold == "Debes", 10,
      ...
      mutate(cal_focus_measurement_lost := ifelse(
        f_focus_measurement_lost == "Debes", 10,
        cal_focus_measurement_lost )) %>%
  
      mutate(cal_focus_measurement_prod := ifelse(
        f_focus_measurement_prod == "gunia|gunia", 70,
        cal_focus_measurement_prod )) %>%
      
      mutate(cal_focus_measurement_sold:= ifelse(
        f_focus_measurement_sold == "gunia|gunia", 70,
        cal_focus_measurement_sold )) %>% 
      
      mutate(cal_focus_measurement_lost := ifelse(
        f_focus_measurement_lost == "gunia|gunia", 70,
        cal_focus_measurement_lost )) 
# 
...
# MOLO EDITS Code for calculating Sunflower Production, sold and lost ####
# Data <- Data %>%
#   
#   # Get the kgs produced
#   mutate(cal_focus_quant_prod_kg=
#            ifelse(f_focus_measurement_prod == "Debes", f_focus_quant_prod*10,
#                   ifelse(f_focus_measurement_prod=="gunia", f_focus_quant_prod*70, f_focus_quant_prod))) %>%
#   # Get the kgs sold
#   mutate(cal_focus_quant_sold_kg = 
#            ifelse(f_focus_measurement_sold == "Debes", f_focus_quant_sold*10, 
#                   ifelse(f_focus_measurement_sold == "gunia", f_focus_quant_sold*10, f_focus_quant_sold))) %>%
#   mutate(cal_focus_quant_lost_kg = 
#            ifelse(f_focus_measurement_lost =="Debes",f_focus_quant_lost*10,
#                   ifelse(f_focus_measurement_lost =="gunia", f_focus_quant_lost*70,f_focus_quant_lost))) %>%
#   mutate(cal_focus_price= f_focus_price)  
# 
  ...
  labour_options <- c("landprep","planting", "cropmaint", 
  ...
  Data <- Data %>%
    mutate(cal_labour_otheractivity_day_costs= f_labour_otheractivity_nrdays * f_labour_otheractivity_paymentperday * f_labour_otheractivity_nrhiredpeople)
  ...
  Data <- Data %>% 
    mutate(cal_labour_cost = Data %>%
             ...
             rowSums(na.rm=TRUE))     %>%
  #Add the costs for "otheractivity" that we calculated before
  mutate(cal_labour_otheractivity_day_costs = ifelse(is.na(cal_labour_otheractivity_day_costs), 0, cal_labour_otheractivity_day_costs))%>%
    mutate(cal_labour_cost = cal_labour_cost + cal_labour_otheractivity_day_costs) %>%
    select(-ends_with("day_costs"))
...
Data <- Data %>%
    ...
    mutate(f_transport = ifelse(is.na(f_transport), 0, f_transport)) 
  
  Data <- Data %>%
    mutate(cal_focus_cost = cal_labour_cost + cal_inputs_costs + f_transport) %>%
    mutate(cal_focus_cost = ifelse(is.na(cal_focus_cost), 0, cal_focus_cost)) %>%
    

  ##  ---- 3f) Calc: Net-income focus crop ----                
  
  mutate(cal_focus_income = cal_focus_revenue - cal_focus_cost) %>%
  mutate(cal_focus_income = ifelse(is.na(cal_focus_income), 0, cal_focus_income))
...
Data <- Data %>%
  ...
  mutate(cal_othermaincrop_income = cal_othermaincrop_1_inc_sold - cal_othermaincrop_1_cost + cal_othermaincrop_2_inc_sold - cal_othermaincrop_2_cost  ) %>%
    mutate(cal_othermaincrop_income = ifelse(is.na(cal_othermaincrop_income), 0, cal_othermaincrop_income)) %>%  
  ...
  mutate(cal_other_crop_income = ifelse(is.na(cal_other_crop_income), 0, cal_other_crop_income)) %>%
  ...
  # NO DATA ON LIVESTOCK LABOUR COST 
    #Livestock Labour cost
    # Data <- Data %>%
...
#We need to keep the variable capturing the highest level of location (pi_location_cascade_first..) for the Farmfit portal. Check what variable that is for each case, this differs.
    ...
    Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_region) %>%  ### the variable on the right differs from case to case!!!
          select(
...
# Error here #
  ...
  if("identifier" %in% colnames(Data)){     
    ...
    for (nr in c(1:15)){
      ...
      Data <- Data %>%
        ...
        mutate(
          # !!sym(hh_member_birthyear_nr) := ifelse(id == nr, hh_member_birthyear, NA),
        ...
        #Delete the rows that do not apply
        #mutate(
          # !!sym(hh_member_birthyear_nr) := ifelse(id >1 , NA, !!sym(hh_member_birthyear_nr) ),
  ...
  vars_dashboard$compare <- vars_dashboard$LATEST %in% data$variable 
