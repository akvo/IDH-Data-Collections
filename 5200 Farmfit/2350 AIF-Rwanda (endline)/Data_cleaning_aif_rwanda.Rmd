...
#If crop size is higher than farm size, interchange

identifier_to_replace <- subset(Data, f_focus_crop_size > f_size, select = c(identifier, f_size, f_focus_crop_size))
Data <- Data %>%
 mutate(f_focus_crop_size = ifelse(identifier %in% identifier_to_replace$identifier, identifier_to_replace$f_size, f_focus_crop_size),
     f_size = ifelse(identifier %in% identifier_to_replace$identifier, identifier_to_replace$f_focus_crop_size, f_size)
     )
...
Data <- Data %>%
  ...
  mutate(f_unit_land = ifelse(f_size>=1000 & f_unit_land != "squared meters", "squared meters",
                                  f_unit_land),
         f_size=ifelse(f_size>=100 & f_unit_land == "acres", f_size / 10,
                       ifelse(f_size>=100 & f_unit_land == "hectares", f_size / 10,
                       f_size)),
         f_focus_crop_size=ifelse(f_focus_crop_size>=100 & f_unit_land == "acres", f_focus_crop_size / 10,
                                  ifelse(f_focus_crop_size>=100 & f_unit_land == "hectares", f_focus_crop_size / 10,
                       f_focus_crop_size)),
  ) 

#values up to 1000 sqm convert to acres to avoid being deleted by outlier function
identifier_to_replace <- subset(Data, f_size>=1000 & f_unit_land == "squared meters", select = identifier)
Data <- Data %>%
  mutate(f_unit_land=ifelse(identifier %in% identifier_to_replace$identifier, "acres", f_unit_land),
         f_size=ifelse(identifier %in% identifier_to_replace$identifier, f_size/4047, f_size),
         f_focus_crop_size=ifelse(identifier %in% identifier_to_replace$identifier, f_focus_crop_size/4047, f_focus_crop_size)
  )
...
Data <-Data %>%
  mutate(
    #production
    f_focus_measurement_prod = ifelse(identifier%in% c("fvn3mf5g10n1", "b2s069vt91fh","7xnk8jm61mje", "s7sb6sg57rks","ujyrg2e62ksu","b4a05a79kd61","rkyvca7b6bjr") ,"90 kg bag", 
                                      ifelse(identifier %in% c("xxhn497d79e6","pj0ge1eqerrd","yk4x75v1805a","m7ppv58yrmun"),"50 kg bag",
    ...
    #sold
    f_focus_measurement_sold=ifelse(identifier%in%c("jmvxjaa8stw9","ewvufrv186ws","vr8ehbs0s6tc","0ncageb2s4ju"), "kg",
                                    ifelse(identifier%in%c("s7sb6sg57rks","ujyrg2e62ksu","b4a05a79kd61","rkyvca7b6bjr"), "90 kg bag",
                                           ifelse(identifier %in% c("yk4x75v1805a","m7ppv58yrmun"),"50 kg bag",
    ...
    f_focus_measurement_sold = ifelse(f_focus_measurement_sold %in% c("100","100 bags kg","100 kg","100 kg bags","100 kgs","100kg"),"100 kg bag",
                                      ifelse(f_focus_measurement_sold=="110kg","110 kg bag",
                                            ...
                                            ifelse(f_focus_measurement_sold %in% c("1kg", "kilogram", "kilograms", "kg umunzani","umunzani","umunzani kg"),"kg",
                                                                 ifelse(f_focus_measurement_sold =="50kg bag","50 kg bag",
                                      f_focus_measurement_sold))))))))),
                                                       ...
                                                       ifelse(f_focus_own_consumption_measurement == "130 kgs", "130 kg bag",
                                                              ifelse(f_focus_own_consumption_measurement == "140 kgs", "140 kg bag",
                                                                     ifelse(f_focus_own_consumption_measurement %in% c("kilogram","kilograms", "umunzani"), "kg",
                                                                            ifelse(f_focus_own_consumption_measurement =="50kg bag","50 kg bag",
...
#replace price by NA as the miss quantity sold and produced
Data <- Data %>%
  mutate(f_focus_price = ifelse(identifier %in% c("6uqysv95k2dp","c8e63r9bjent"), NA, f_focus_price))
  ...
  mutate(f_focus_price = ifelse(identifier =="hmy0dx5tyndg", 20000, 
                                      ifelse(identifier=="rvqudu81ud6a", 120000, 
                                            ifelse(identifier=="dedx7uxqqcwq", 325000, 
                                                   ...
                                                   #oultliers
                                                   ifelse(identifier %in% c("emj4key9puef","2e00npfbragd","k7mrepywua1u", "x4k0q51788un","a30w0ra75eq9"), median_price_100kg$median_price_100kg,  
                                f_focus_price)))))
...
#other outliers with unit 50kg and 90 kg
median_price_50kg<- Data %>%
  filter(f_focus_measurement_sold == "50 kg bag") %>%
  summarize(median_price_50kg = median(f_focus_price, na.rm = TRUE))

median_price_90kg<- Data %>%
  filter(f_focus_measurement_sold == "90 kg bag") %>%
  summarize(median_price_90kg = median(f_focus_price, na.rm = TRUE))

Data <- Data %>%
  mutate(f_focus_price = ifelse(identifier == "vtvs1k8uqrta", median_price_90kg$median_price_90kg, f_focus_price),
         f_focus_price = ifelse(identifier == "0eww9f32bqpb", median_price_50kg$median_price_50kg, f_focus_price),
         )
