---
title: "Data_clearning_Coscharis_endline"
...
```{r}
data_filename <- "DATA_ANALYSIS-endline-Coscharis.xlsx"
survey_filename <- "SURVEY_FORM-endline-Coscharis.xlsx"
```
...
Data <- Data %>%
  ...
  # Remove irrelevant columns
  # ONLY DELETE SUBMITTER AFTER THE PRODUCTIVITY CHECK! YOU WANT THIS VARIABLE IN FOR OUTLIER CHECKING
...
# Combine unit columns
  ...
  # Convert land size to acres if in hectares
   ...
   mutate(f_focus_crop_size_acre = case_when(
    ...
    f_unit_land == "chain" ~ f_focus_crop_size/10,
  ...
  mutate(f_size_hectare = case_when(
    ...
    f_unit_land == "chain" ~ f_size / 24.71,
  ...
  mutate(f_focus_crop_size_hectare = case_when(
    ...
    f_unit_land == "chain" ~ f_focus_crop_size / 24.71,
...
```{r}
# Farm size
 #Focus crop crop higher than farm size. We decided to put them as NA
Data<-Data %>%
  mutate(
    f_focus_crop_size=case_when(
      identifier=="50ykdn87nqjn"~ NA, TRUE ~f_focus_crop_size
                                ),
    f_focus_crop_size=case_when(
      identifier=="khxjgtd7y4rr"~ NA, TRUE ~f_focus_crop_size
                                ),
        f_focus_crop_size=case_when(
      identifier=="djcsrpq1vgc8"~ NA, TRUE ~f_focus_crop_size
                                ),
            f_focus_crop_size=case_when(
      identifier=="wc8n97a2cf1b"~ NA, TRUE ~f_focus_crop_size
                                ),
                f_focus_crop_size=case_when(
      identifier=="e84rdbavvbf0"~ NA, TRUE ~f_focus_crop_size
                                ),
                    f_focus_crop_size=case_when(
      identifier=="53sj1nbpck0h"~ NA, TRUE ~f_focus_crop_size
                                ),
                        f_focus_crop_size=case_when(
      identifier=="6cxfm0uc7brr"~ NA, TRUE ~f_focus_crop_size
                                )
  ) 
...
#  mutate(
#    f_focus_crop_size = case_when(
#      identifier %in% c("50ykdn87nqjn", "khxjgtd7y4rr", "djcsrpq1vgc8", "wc8n97a2cf1b", "e84rdbavvbf0", "53sj1nbpck0h", #"6cxfm0uc7brr") ~ NA,
 #     TRUE ~ f_focus_crop_size
 #   )
 # )
...
Data <-Data %>%
  mutate(
    #production
    #change measurement unit to 300 kilogram bags
    f_focus_measurement_prod = ifelse(identifier=="4k0nyygt7uqj","300 kilogram bags", f_focus_measurement_prod),
    
  #sold
    #change measurement unit of quantity sold from kg, 100kg to 300 kilogram bags
  f_focus_measurement_sold=ifelse(f_focus_measurement_sold=="kg" | f_focus_measurement_sold=="", "300 kilogram bags",f_focus_measurement_sold),
  
    
  #consumption
    #change measurement unit of quantity consumed to the one as the quantity sold
 f_focus_own_consumption_measurement=ifelse(f_focus_own_consumption_measurement=="kgs" & f_focus_own_consumption <=170, f_focus_measurement_sold, f_focus_own_consumption_measurement),
  
  #f_focus_own_consumption_measurement=case_when( 
  # identifier%in%c("khxjgtd7y4rr", "4ycx4junrve3", "50ykdn87nqjn", "9pxq833jkyuv","a3p0a1d8jyya", "4ysw47a9y8cm", "uyhvxc003ud7", #"qm0krx0d5mkk", "79a13wx58f5f", "pt6y3t087f81","jscpe1fq34d0", "ry97j8djakqa", "ukfu8ma53gvn", "7726f3k58b34", "k68q6g1p6c7s", #"q6k1kjxpwmmd", "q02mve454a5x", "pbs6wc43vrs5", "pjk6p45n3kn6", "fyv9k2fcpscg", "b749fj10rwmf", "6yjupmhxvcjj", "njy0g25rhp7y", #"nghset89mea7", "dhdt05saufm4", "jaa3qyurryp0", "1t25cfwgfwxp", "f1vfbjup6af4", "g1w78xbqc4gu", "t7upep6x0xth", "fftbu3edrahe", #"mrsw7981c8pn", "ffkhgn25xak6", "cv0849y396mm", "6ym485acpkau", "vct8672psf9k", "qs045a4cau0k", "kh2jfyr1x9f7", "g2d254897s0d", #"m8y4g2acvdce", "58fdr1ru0kw1", "eksnr87n98g1", "v1aj8d4uy176", "43mbb3pj1j3y", "c869t9qe2c1d", "04ec3tubq7xs", "rwh9hw2qae80", #"uds69ram9vug","3ksy9x284qud","j3tsrnunayhk","4k0nyygt7uqj","ac0c8k96qg1r","bts29gqyajjw","wpqd8q3r1s4p","2armc99ufs2m","cm3npvbsjw2m") ~ #f_focus_measurement_sold, TRUE ~ f_focus_own_consumption_measurement), 
  

  #change those entry consumption quantity from kgs to  300 kilogram bags. This will be done by changing the unit in 300 kilogram bags and after divide the consume quantity by 300

f_focus_own_consumption_measurement=case_when(
  identifier%in%c("x3cmnyg1pajm", "q02mve454a5x", "6aqdrmshdfbn", "q02mve454a5x", "vkbn7je1k359", "tr6jjegkjc8y", "88g50mgbxxn5", "dfvdsr489cde","5pfs05s92ust") ~ "300 kilogram bags",TRUE ~ f_focus_own_consumption_measurement),


f_focus_own_consumption=case_when(
  identifier%in% c("x3cmnyg1pajm", "q02mve454a5x", "6aqdrmshdfbn", "q02mve454a5x", "vkbn7je1k359", "tr6jjegkjc8y", "88g50mgbxxn5", "dfvdsr489cde","5pfs05s92ust") ~ f_focus_own_consumption/300,TRUE ~ f_focus_own_consumption),


# change the quantity consumed to NA since quantity produced=quantity sold
f_focus_own_consumption=ifelse(identifier=="taqwye6cxcv8",NA, f_focus_own_consumption),
f_focus_own_consumption_measurement=ifelse(identifier=="taqwye6cxcv8"," ", f_focus_own_consumption_measurement),
  

 #Lost
  #change the measurment unit to 300 kilogram bags
f_focus_measurement_lost=case_when(
    identifier%in%c("e84rdbavvbf0", "53sj1nbpck0h", "ktru1n9v0k1r", "wc8n97a2cf1b", "khxjgtd7y4rr", "4ycx4junrve3", "6ajvqwewvv5k", "50ykdn87nqjn", "f6em8fqtmw2a", "fyv9k2fcpscg", "b749fj10rwmf", "p8nhnvpbs4re", "dhdt05saufm4", "jaa3qyurryp0", "1t25cfwgfwxp", "f1vfbjup6af4", "t7upep6x0xth", "ffkhgn25xak6", "uds69ram9vug","x3cmnyg1pajm","6aqdrmshdfbn") ~ "300 kilogram bags", TRUE ~ f_focus_measurement_lost),


  #convert the quantity to 300 kilogram bags
f_focus_quant_lost=case_when(
  identifier=="6aqdrmshdfbn" ~ f_focus_quant_lost/300,TRUE ~f_focus_quant_lost
)

)

#Data <-Data%>%
#mutate (f_focus_quant_lost=ifelse(identifier== "6aqdrmshdfbn", f_focus_quant_lost/300,f_focus_quant_lost))
...
median_price_300kgs<- Data %>%
  filter(f_focus_measurement_sold == "300 kilogram bags") %>%
  summarize(median_price_300kgs = median(f_focus_price, na.rm = TRUE))


#lets define the id where price need to be replaced by median of 300 kilogram bags
identifiers_to_replace <- c("xvpfkd0g93y9","e84rdbavvbf0", "53sj1nbpck0h", "wc8n97a2cf1b", "9fsepwr1a0hu", "khxjgtd7y4rr",
                            "4ycx4junrve3", "6ajvqwewvv5k", "kq13wby5xd6k", "9pxq833jkyuv", "ehfcy7n5e0tm", "a3p0a1d8jyya",
                            "mtcjq6ff781m" )

#now replace
Data <- Data %>%
  mutate(f_focus_price = ifelse(
    identifier %in% identifiers_to_replace & !is.na(median_price_300kgs$median_price_300kgs),
    median_price_300kgs$median_price_300kgs,
    f_focus_price
  ))

#identifier that need their f_focus_price to be convert to 300kgs price

#Data <-Data%>%
#  mutate(
#    f_focus_price=case_when(
#    identifier=="50ykdn87nqjn"~f_focus_price*300,TRUE~ f_focus_price
#  ),
#   f_focus_price=case_when(
#    identifier=="j65tkjfqspap"~f_focus_price*300,TRUE~ f_focus_price
#  ),
#    f_focus_price=case_when(
#    identifier=="54macs1psyp4"~f_focus_price*300,TRUE~ f_focus_price
#  )
 # )


#identifier that need their f_focus_price to be convert to the median price of 300 kgs bags
Data <-Data%>%
  mutate(f_focus_price=case_when(identifier%in%c("50ykdn87nqjn", "j65tkjfqspap","54macs1psyp4") ~ median_price_300kgs$median_price_300kgs, TRUE ~ f_focus_price))
```

```{r}
# Harmonistation of the different measurement unit

Data <- Data %>%
  mutate(
f_focus_measurement_prod = ifelse(f_focus_measurement_prod=="300 kgs" | f_focus_measurement_prod=="300kgbag"| f_focus_measurement_prod=="300 kilogram bags", "300 kgs bags", f_focus_measurement_prod),

f_focus_measurement_sold= ifelse(f_focus_measurement_sold =="300kg" |f_focus_measurement_sold == "300 kilogram bags","300 kgs bags", f_focus_measurement_sold ),

f_focus_own_consumption_measurement=ifelse(f_focus_own_consumption_measurement=="300"|f_focus_own_consumption_measurement== "300kg bag"| f_focus_own_consumption_measurement=="300kg"| f_focus_own_consumption_measurement=="300 kg bag"|f_focus_own_consumption_measurement=="300kgs"| f_focus_own_consumption_measurement=="300 kilogram bags", "300 kgs bags",f_focus_own_consumption_measurement),

f_focus_measurement_lost=ifelse(f_focus_measurement_lost=="300 kilogram bags","300 kgs bags", f_focus_measurement_lost )
...
```{r}
#The entry below has some abnormal information. So we prefer to delete tehm
Data <-Data %>%
  mutate(
f_focus_quant_prod=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r",NA,f_focus_quant_prod),
f_focus_measurement_prod=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r","",f_focus_measurement_prod),

f_focus_quant_sold=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r",NA,f_focus_quant_sold),
f_focus_measurement_sold=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r","",f_focus_measurement_sold),
f_focus_price=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r",NA, f_focus_price),


f_focus_own_consumption=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r",NA,f_focus_own_consumption),
f_focus_own_consumption_measurement=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r","",f_focus_own_consumption_measurement),

f_focus_quant_lost=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r",NA,f_focus_quant_lost),
f_focus_measurement_lost=ifelse(identifier=="f6em8fqtmw2a" |identifier =="ktru1n9v0k1r","",f_focus_measurement_lost)
...
# Some values on quantities produced and sold have been considered as outlier and deleted, but they are just exact figures from big producers.

#quantity produced

Data <-Data%>%
  mutate(
   f_focus_quant_prod =case_when(
    identifier=="39ypy5m5dk2y"~200,TRUE~ f_focus_quant_prod
  ),
   f_focus_quant_prod=case_when(
    identifier=="tp2tndgqtuqg"~200,TRUE~ f_focus_quant_prod
  ),
    f_focus_quant_prod=case_when(
    identifier=="sexa8v1x8vtv"~350,TRUE~ f_focus_quant_prod
  ),
   f_focus_quant_prod=case_when(
    identifier=="6tvrbum79yr0"~230,TRUE~ f_focus_quant_prod
  ),
  f_focus_quant_prod=case_when(
    identifier=="43mbb3pj1j3y"~200,TRUE~ f_focus_quant_prod
  ),
  f_focus_quant_prod=case_when(
    identifier=="1awcfcyqxh8u"~300,TRUE~ f_focus_quant_prod
  ),
    f_focus_quant_prod=case_when(
    identifier=="9mws0955721k"~219,TRUE~ f_focus_quant_prod
  ),
...
#Quantity sold
   ...
   f_focus_quant_sold =case_when(
    identifier=="39ypy5m5dk2y"~190,TRUE~ f_focus_quant_sold 
  ),
   f_focus_quant_sold =case_when(
    identifier=="tp2tndgqtuqg"~290,TRUE~ f_focus_quant_sold 
  ),
    f_focus_quant_sold =case_when(
    identifier=="sexa8v1x8vtv"~330,TRUE~ f_focus_quant_sold 
  ),
   f_focus_quant_sold =case_when(
    identifier=="6tvrbum79yr0"~230,TRUE~ f_focus_quant_sold 
  ),
  f_focus_quant_sold =case_when(
    identifier=="43mbb3pj1j3y"~180,TRUE~ f_focus_quant_sold 
  ),
  f_focus_quant_sold =case_when(
    identifier=="1awcfcyqxh8u"~280,TRUE~ f_focus_quant_sold 
  ),
    f_focus_quant_sold =case_when(
    identifier=="9mws0955721k"~110,TRUE~ f_focus_quant_sold 
  )
...
Data <- Data %>%
  ...
  mutate(
    ...
    cal_focus_quant_prod_kg_season_1 = ifelse(f_focus_rev_timeperiod == "Season 1 - April (Wet season)", 
cal_focus_quant_prod_kg, NA),
    cal_focus_quant_prod_kg_season_2 = ifelse(f_focus_rev_timeperiod == "Season 2 - September  (Dry season)", 
cal_focus_quant_prod_kg, NA),

#

    cal_focus_quant_sold_kg_whole_year = ifelse(f_focus_rev_timeperiod == "Past 12 months (whole year)", cal_focus_quant_prod_kg, NA),
    cal_focus_quant_sold_kg_season_1 = ifelse(f_focus_rev_timeperiod == "Season 1 - April (Wet season)", 
cal_focus_quant_prod_kg, NA),
    cal_focus_quant_sold_kg_season_2 = ifelse(f_focus_rev_timeperiod == "Season 2 - September  (Dry season)", 
cal_focus_quant_prod_kg, NA),
...
#
 ...
 cal_focus_quant_own_consumption_kg_season_1 = ifelse(f_focus_rev_timeperiod == "Season 1 - April (Wet season)", cal_focus_measurement_own_consumption, NA),
 cal_focus_quant_own_consumption_kg_season_2 = ifelse(f_focus_rev_timeperiod == "Season 2 - September  (Dry season)", cal_focus_measurement_own_consumption, NA),

#

 cal_focus_quant_lost_kg_whole_year = ifelse(f_focus_rev_timeperiod == "Past 12 months (whole year)", cal_focus_quant_lost_kg, NA),
 cal_focus_quant_lost_kg_season_1 = ifelse(f_focus_rev_timeperiod == "Season 1 - April (Wet season)", cal_focus_quant_lost_kg, NA),
 cal_focus_quant_lost_kg_season_2 = ifelse(f_focus_rev_timeperiod == "Season 2 - September  (Dry season)", cal_focus_quant_lost_kg, NA),
...
```{r, include = FALSE}
Data <- Data %>%
          rename(location_cascade_region = pi_location_cascade_community) %>%  ### the variable on the right differs from case to case!!!
          select(
...
```{r}
Data_baseline<-read_excel("DATA_ANALYSIS-baseline-Coscharis.xlsx")
...
write.xlsx(pi_info, file="Pi_coscharis_baseline.xlsx")
...
```{r}
baseline_farmers <- read_excel("Pi_conscharis_baseline.xlsx") 
